...
class PostgresConnectionMonitor:
    ...
    async def analyze_connections(self) -> PoolMetrics:
        ...
        recommendations = self._generate_recommendations(
            total, active, idle, idle_in_transaction, long_running, utilization, overflow_in_use
        )
    ...
    def _generate_recommendations(
        ...
        if utilization < 20 and total > 5:
            recommendations.append(
                f"💡 Pool subutilizado ({utilization:.1f}%). Considerar reducir pool_size para liberar recursos."
            )
...
"""
    ...
    async def monitor_loop(self, interval_seconds: int = 10, duration_seconds: int = 60):
        ...
        while datetime.now() < end_time:
            ...
            print(f"\n⏰ {metrics.timestamp}")
            print(
                f"📊 Conexiones: {metrics.total_connections} total, "
                f"{metrics.active_connections} active, "
                f"{metrics.idle_connections} idle"
            )
            print(
                f"📈 Utilización: {metrics.pool_utilization_percent}% "
...
async def main():
    """Función principal."""
    parser = argparse.ArgumentParser(description="Monitoreo de Connection Pool PostgreSQL")
    ...
    parser.add_argument("--database", default="agente_hotel", help="Database name (default: agente_hotel)")
    parser.add_argument("--pool-size", type=int, default=10, help="Configured pool size (default: 10)")
    parser.add_argument("--max-overflow", type=int, default=20, help="Configured max overflow (default: 20)")
    ...
    parser.add_argument("--duration", type=int, default=60, help="Watch duration in seconds (default: 60)")
    parser.add_argument("--output", default=".playbook/connection_pool_report.json", help="Output file for JSON report")
    ...
    try:
        ...
        if args.watch:
            ...
            await monitor.monitor_loop(interval_seconds=args.interval, duration_seconds=args.duration)
        else:
            # Análisis único

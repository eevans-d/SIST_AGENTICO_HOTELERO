...
class MultilingualTester:
            ...
            for message in messages:
                try:
                    ...
                    if detected_lang == expected_lang:
                        ...
                        error = {"message": message, "expected": expected_lang, "detected": detected_lang}
                    ...
                    self.results["language_detection"]["errors"].append({"message": message, "error": str(e)})

        accuracy = (
            self.results["language_detection"]["correct"] / self.results["language_detection"]["total"] * 100
            if self.results["language_detection"]["total"] > 0
            else 0
        )
            ...
            for i, message in enumerate(messages):
                ...
                try:
                    ...
                    self.results["intent_recognition"]["errors"].append(
                        {"message": message, "language": lang, "error": str(e)}
                    )

        accuracy = (
            self.results["intent_recognition"]["correct"] / self.results["intent_recognition"]["total"] * 100
            if self.results["intent_recognition"]["total"] > 0
            else 0
        )
        ...
        for expected_lang, messages in TEST_MESSAGES.items():
            for i, message in enumerate(messages[:2]):  # Test first 2 messages per language
                ...
                try:
                    ...
                    e2e_results["errors"].append({"message": message, "error": str(e)})

        accuracy = e2e_results["correct"] / e2e_results["total"] * 100 if e2e_results["total"] > 0 else 0

        logger.info(f"End-to-end accuracy: {accuracy:.1f}% ({e2e_results['correct']}/{e2e_results['total']})")
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        total_correct = lang_detection["correct"] + intent_recognition["correct"] + e2e_results["correct"]
        total_tests = lang_detection["total"] + intent_recognition["total"] + e2e_results["total"]
        ...
        logger.info(
            f"Language Detection: {lang_detection['correct']}/{lang_detection['total']} ({lang_detection['correct'] / lang_detection['total'] * 100:.1f}%)"
        )
        logger.info(
            f"Intent Recognition: {intent_recognition['correct']}/{intent_recognition['total']} ({intent_recognition['correct'] / intent_recognition['total'] * 100:.1f}%)"
        )
        logger.info(
            f"End-to-End: {e2e_results['correct']}/{e2e_results['total']} ({e2e_results['correct'] / e2e_results['total'] * 100:.1f}%)"
        )
        logger.info("=" * 60)

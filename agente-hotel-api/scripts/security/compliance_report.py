...
class ComplianceReportGenerator:
    ...
    def __init__(self, project_root: Path):
        ...
        self.severity_weights = {"CRITICAL": 10, "HIGH": 5, "MEDIUM": 2, "LOW": 1}
    ...
    def calculate_risk_score(self) -> RiskAssessment:
        ...
        total_weight = sum(count * self.severity_weights[severity] for severity, count in severity_counts.items())
    ...
    def update_slo_status(self, risk_assessment: RiskAssessment, compliance_metrics: ComplianceMetrics):
        ...
        self.slos["critical_findings"]["status"] = (
            "PASS" if risk_assessment.critical_count <= self.slos["critical_findings"]["max"] else "FAIL"
        )

        # High findings SLO
        self.slos["high_findings"]["current"] = risk_assessment.high_count
        self.slos["high_findings"]["status"] = (
            "PASS" if risk_assessment.high_count <= self.slos["high_findings"]["max"] else "FAIL"
        )

        # Compliance score SLO
        self.slos["compliance_score"]["current"] = compliance_metrics.owasp_score
        self.slos["compliance_score"]["status"] = (
            "PASS" if compliance_metrics.owasp_score >= self.slos["compliance_score"]["min"] else "FAIL"
        )
        ...
        self.slos["hardcoded_secrets"]["status"] = (
            "PASS" if secret_count <= self.slos["hardcoded_secrets"]["max"] else "FAIL"
        )
    ...
    def export_to_json(self, report: ComplianceReport, output_path: Path):
        ...
        data = {
            ...
            "findings_summary": {source: len(findings) for source, findings in report.findings_by_source.items()},
    ...
    def export_to_markdown(self, report: ComplianceReport, output_path: Path):
        ...
        lines.append("**Project**: Agente Hotelero IA")
        lines.append("")
        lines.append("---")
        lines.append("")
        ...
        lines.append(
            f"**Risk Level**: {self._get_risk_emoji(report.risk_assessment.risk_level)} **{report.risk_assessment.risk_level}**"
        )
        ...
        lines.append("|----------|-------|--------|--------|")
        lines.append(
            f"| 🔴 **CRITICAL** | {report.risk_assessment.critical_count} | 10 | {report.risk_assessment.critical_count * 10} |"
        )
        lines.append(
            f"| 🟠 **HIGH** | {report.risk_assessment.high_count} | 5 | {report.risk_assessment.high_count * 5} |"
        )
        lines.append(
            f"| 🟡 **MEDIUM** | {report.risk_assessment.medium_count} | 2 | {report.risk_assessment.medium_count * 2} |"
        )
        lines.append(
            f"| 🟢 **LOW** | {report.risk_assessment.low_count} | 1 | {report.risk_assessment.low_count * 1} |"
        )
        lines.append("")
        ...
        lines.append("|----------|----------|--------------|")
        lines.append(
            f"| **OWASP Top 10 2021** | {report.compliance_metrics.owasp_coverage}% | {report.compliance_metrics.owasp_score}/100 |"
        )
        ...
        for slo_name, slo_data in report.slo_status.items():
            ...
            lines.append(
                f"| {slo_name.replace('_', ' ').title()} | {target} | {slo_data['current']} | {status_emoji} {slo_data['status']} |"
            )
        lines.append("")
        ...
        if report.risk_assessment.critical_count > 0:
            ...
            lines.append("")
            lines.append(
                f"- **{report.risk_assessment.critical_count} CRITICAL findings** must be fixed before deployment"
            )

...
def run_command(command, cwd=None):
    ...
    try:
        result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, cwd=cwd, shell=True)
...
def create_test_splits(nlu_files, output_dir, test_ratio=0.2):
    ...
    for nlu_file in nlu_files:
        ...
        for intent_entry in data["nlu"]:
            ...
            train_data["nlu"].append({"intent": intent_name, "examples": "\n".join(train_examples)})

            if test_examples:  # Solo agregar si hay ejemplos de prueba
                test_data["nlu"].append({"intent": intent_name, "examples": "\n".join(test_examples)})
        ...
        output_files[lang] = {"train": train_file, "test": test_file}
...
def evaluate_models(models, splits, langs, output_dir):
    ...
    if "multilingual" in models:
        ...
        for lang in langs:
            ...
            # Ejecutar evaluación
            eval_cmd = (
                f"rasa test nlu --model {models['multilingual']} --nlu {splits[lang]['test']} --out {lang_eval_dir}"
            )
...
def generate_comparison_report(results, output_dir):
    ...
    if df["model"].str.contains("multilingual").any():
        ...
        if avg_multi > avg_specific:
            summary["conclusion"] = (
                f"El modelo multilingüe supera a los modelos específicos en un promedio de {((avg_multi - avg_specific) / avg_specific) * 100:.2f}% en F1-score."
            )
        else:
            summary["conclusion"] = (
                f"Los modelos específicos de idioma superan al modelo multilingüe en un promedio de {((avg_specific - avg_multi) / avg_multi) * 100:.2f}% en F1-score."
            )
...
def main():
    ...
    parser.add_argument(
        "--rasa-dir", type=str, default="../rasa_nlu", help="Directorio base de Rasa NLU con datos existentes"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--test-ratio", type=float, default=0.2, help="Proporción de datos a usar para pruebas (0.0-1.0)"
    )

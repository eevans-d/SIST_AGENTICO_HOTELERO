...
encryption_operations_total = Counter(
    "encryption_operations_total", "Total encryption operations", ["operation", "algorithm", "status"]
)

encryption_duration_seconds = Histogram(
    "encryption_duration_seconds", "Encryption operation duration", ["operation", "algorithm"]
)
...
class DataEncryptionService:
    ...
    async def _initialize_default_keys(self):
        ...
        for classification in DataClassification:
            ...
            encryption_key = EncryptionKey(
                key_id=key_id, algorithm=algorithm, key_data=key_data, classification=classification
            )
    ...
    async def encrypt_data(
        ...
        try:
            ...
            encryption_duration_seconds.labels(operation="encrypt", algorithm=algorithm.value).observe(duration)

            encryption_operations_total.labels(operation="encrypt", algorithm=algorithm.value, status="success").inc()
            ...
            encryption_operations_total.labels(
                operation="encrypt", algorithm=algorithm.value if algorithm else "unknown", status="error"
            ).inc()
    ...
    async def decrypt_data(self, encrypted_data: EncryptedData) -> bytes:
        ...
        try:
            ...
            # Update metrics
            duration = asyncio.get_event_loop().time() - start_time
            encryption_duration_seconds.labels(operation="decrypt", algorithm=encrypted_data.algorithm.value).observe(
                duration
            )

            encryption_operations_total.labels(
                operation="decrypt", algorithm=encrypted_data.algorithm.value, status="success"
            ).inc()
            ...
            encryption_operations_total.labels(
                operation="decrypt", algorithm=encrypted_data.algorithm.value, status="error"
            ).inc()
    ...
    async def _decrypt_aes_gcm(self, encrypted_data: EncryptedData, key: bytes) -> bytes:
        ...
        cipher = Cipher(
            algorithms.AES(key), modes.GCM(encrypted_data.nonce, encrypted_data.tag), backend=default_backend()
        )
    ...
    async def _decrypt_chacha20(self, encrypted_data: EncryptedData, key: bytes) -> bytes:
        ...
        cipher = Cipher(algorithms.ChaCha20(key, encrypted_data.nonce), None, backend=default_backend())
        decryptor = cipher.decryptor()
    ...
    def _get_default_key_id(self, classification: DataClassification, algorithm: EncryptionAlgorithm) -> str:
    ...
    async def create_encryption_key(
        self, classification: DataClassification, algorithm: Optional[EncryptionAlgorithm] = None
    ) -> str:
    ...
    async def rotate_key(self, key_id: str) -> str:
        ...
        new_key_id = await self.create_encryption_key(old_key.classification, old_key.algorithm)
    ...
    async def encrypt_pii_data(self, pii_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pii_fields = ["email", "phone", "ssn", "passport_number", "credit_card", "address", "date_of_birth"]

        for field_name, value in pii_data.items():
            if field_name in pii_fields and value:
                encrypted_data = await self.encrypt_data(str(value), DataClassification.RESTRICTED)
                encrypted_pii[field_name] = encrypted_data.to_dict()
            else:
                encrypted_pii[field_name] = value
    ...
    async def decrypt_pii_data(self, encrypted_pii: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field_name, value in encrypted_pii.items():
            if isinstance(value, dict) and "encrypted_data" in value:
                ...
                decrypted_pii[field_name] = decrypted_bytes.decode("utf-8")
            else:

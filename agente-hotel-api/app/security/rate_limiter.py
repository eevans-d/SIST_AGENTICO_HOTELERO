...
rate_limit_operations_total = Counter(
    ...
    "Total rate limiting operations",
    ["limiter_type", "result"]
)
...
active_rate_limits_gauge = Gauge(
...
class AdvancedRateLimiter:
    ...
    async def check_rate_limit(self, rule_type: RateLimitRule, 
                             source_ip: str,
                             user_id: Optional[str] = None,
    ...
    def _get_window_key(self, rule_type: RateLimitRule, source_ip: str, 
                       timestamp: datetime, window_size: int) -> str:
    ...
    async def _record_violation(self, rule_type: RateLimitRule, source_ip: str,
                              user_id: Optional[str], request_count: int,
                              timestamp: datetime):
        ...
        self.violation_history[source_ip] = [
            v for v in self.violation_history[source_ip]
    ...
    async def _update_attack_detection(self, source_ip: str, endpoint: str, timestamp: datetime):
        ...
        self.request_timeline = [
            req for req in self.request_timeline
    ...
    async def _detect_volumetric_attack(self, timestamp: datetime):
        ...
        if requests_per_second > self.ddos_thresholds["requests_per_second"]:
            ...
            if attack_id not in self.attack_signatures:
                ...
                logger.critical(f"Volumetric attack detected: {requests_per_second:.1f} req/s from {len(source_ips)} IPs")
    ...
    async def _detect_distributed_attack(self, timestamp: datetime):
        ...
        if len(unique_ips) > self.ddos_thresholds["unique_ips_threshold"]:
            ...
            if attack_id not in self.attack_signatures:
                ...
                ddos_attacks_detected_total.labels(
                    attack_type=AttackType.DISTRIBUTED_ATTACK.value,
                    source="distributed"
                ).inc()
            ...
            if len(requests) > 50:  # Suspicious if more than 50 requests in 5 minutes
                ...
                if len(endpoints) > 10:  # Accessing many different endpoints
                    ...
                    if attack_id not in self.attack_signatures:
                        ...
                        ddos_attacks_detected_total.labels(
                            attack_type=AttackType.APPLICATION_LAYER_ATTACK.value,
                            source=ip
                        ).inc()
        ...
        recent_violations = [
            {
                ...
                "request_count": v.requests_count
            }
    ...
    async def cleanup_expired_data(self):
        ...
        for ip in self.violation_history:
            ...
            self.violation_history[ip] = [
                v for v in self.violation_history[ip]
                if v.timestamp > cutoff_time
            ]

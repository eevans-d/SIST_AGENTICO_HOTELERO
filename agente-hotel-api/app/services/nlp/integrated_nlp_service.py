...
# Prometheus metrics
nlp_service_requests_total = Counter("nlp_service_requests_total", "Total NLP service requests", ["endpoint", "status"])

nlp_service_processing_time = Histogram(
    "nlp_service_processing_time_seconds", "NLP service processing time", ["operation"]
)

nlp_service_active_sessions = Gauge("nlp_service_active_sessions", "Number of active NLP sessions")

nlp_conversation_length = Summary("nlp_conversation_length_messages", "Length of conversations in messages")
...
class IntegratedHotelNLPService:
    ...
    async def process_message(self, request: NLPServiceRequest) -> NLPServiceResponse:
        ...
        try:
            ...
            prediction = await self.nlp_engine.predict_intent(request.message, request.context_metadata)

            # Step 2: Context processing
            processing_result = await self.context_processor.process_prediction(
                request.session_id, prediction, request.message
            )
            ...
            # Update metrics
            nlp_service_requests_total.labels(endpoint="process_message", status="success").inc()

            nlp_service_processing_time.labels(operation="full_processing").observe(processing_time)
            ...
            logger.info(
                f"Message processed successfully - Session: {request.session_id}, "
            ...
            nlp_service_requests_total.labels(endpoint="process_message", status="error").inc()
            ...
            error_response_text = await self.response_generator.generate_error_response(
                "system_error", self.context_processor.get_or_create_context(request.session_id)
            )
    ...
    async def get_conversation_summary(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if summary:
                nlp_service_requests_total.labels(endpoint="get_summary", status="success").inc()

            return summary

        except Exception as e:
            nlp_service_requests_total.labels(endpoint="get_summary", status="error").inc()
    ...
    async def end_conversation(self, session_id: str, reason: str = "user_ended") -> bool:
        ...
        try:
            ...
            if context:
                ...
                logger.info(
                    f"Conversation ended - Session: {session_id}, Reason: {reason}, Messages: {context.message_count}"
                )

            nlp_service_requests_total.labels(endpoint="end_conversation", status="success").inc()

            return True

        except Exception as e:
            nlp_service_requests_total.labels(endpoint="end_conversation", status="error").inc()
    ...
    async def get_intent_suggestions(self, partial_message: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            nlp_service_requests_total.labels(endpoint="get_suggestions", status="success").inc()

            return suggestions

        except Exception as e:
            nlp_service_requests_total.labels(endpoint="get_suggestions", status="error").inc()
    ...
    async def analyze_conversation_patterns(self, time_range_hours: int = 24) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "escalation_rate": sum(1 for ctx in active_contexts if ctx.urgency_level == "high")
                / total_conversations,
            }

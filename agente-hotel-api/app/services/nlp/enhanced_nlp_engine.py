...
class HotelNLPEngine:
    ...
    def __init__(self, redis_client: Optional[redis.Redis] = None):
        ...
        self.intent_patterns = {
            ...
            IntentType.CANCEL_RESERVATION: [r"cancel.*reserva", r"anular.*reserva", r"eliminar.*reserva"],
            IntentType.ROOM_SERVICE: [
                r"room.*service",
                r"servicio.*habitación",
                r"pedir.*comida",
                r"quiero.*comer",
                r"menú",
            ],
            IntentType.CHECK_IN: [r"check.*in", r"registr", r"llegada", r"entrada"],
            IntentType.CHECK_OUT: [r"check.*out", r"salida", r"cuenta", r"factura"],
            IntentType.COMPLAINT: [r"problema", r"queja", r"molest", r"reclam", r"no.*funciona"],
        }
    ...
    def _setup_entity_patterns(self):
        ...
        date_patterns = [
            ...
            [{"TEXT": {"REGEX": r"\d{1,2}-\d{1,2}-\d{4}"}}],
            [{"LOWER": {"IN": ["hoy", "mañana", "pasado"]}}, {"LOWER": {"IN": ["mañana", "", ""]}}],
        ]
    ...
    async def predict_intent(self, text: str, context: Optional[Dict] = None) -> IntentPrediction:
        ...
        try:
            ...
            # Update metrics
            confidence_level = (
                "high" if intent_pred.confidence > 0.8 else "medium" if intent_pred.confidence > 0.5 else "low"
            )
            nlp_intent_predictions_total.labels(
                intent=intent_pred.intent.value, confidence_level=confidence_level, language=intent_pred.language
            ).inc()

            nlp_processing_latency.labels(operation="intent_prediction", model_type="hybrid").observe(
                intent_pred.processing_time
            )
        ...
        try:
            ...
            for match_id, start, end in matches:
                ...
                if entity_type:
                    ...
                    nlp_entity_extractions_total.labels(
                        entity_type=entity_type.value, extraction_method="rule_based"
                    ).inc()
            ...
            for ent in doc.ents:
                ...
                if entity_type:
                    ...
                    nlp_entity_extractions_total.labels(
                        entity_type=entity_type.value, extraction_method="spacy_ner"
                    ).inc()
    ...
    def _label_to_entity_type(self, label: str) -> Optional[EntityType]:
        ...
        mapping = {"ROOM_TYPE": EntityType.ROOM_TYPE, "DATE": EntityType.DATE, "PRICE": EntityType.PRICE}
    ...
    async def _cache_result(self, key: str, result: IntentPrediction):
        ...
        try:
            ...
            await self.redis_client.setex(key, self.cache_ttl, json.dumps(result_dict, default=str))
        except Exception as e:
    ...
    async def analyze_conversation_context(self, messages: List[str]) -> Dict[str, Any]:
        ...
        context["dominant_intents"] = sorted(intent_counts.items(), key=lambda x: x[1], reverse=True)[:3]
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        health_status = {"status": "healthy", "components": {}, "metrics": {}, "timestamp": datetime.now().isoformat()}

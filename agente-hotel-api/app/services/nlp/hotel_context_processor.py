...
class HotelContextProcessor:
    ...
    def _initialize_intent_slots(self) -> Dict[IntentType, List[EntitySlot]]:
        ...
        slots = {
            IntentType.BOOK_ROOM: [
                EntitySlot(
                    "checkin_date", EntityType.DATE, True, prompt_message="¿Para qué fecha necesita la habitación?"
                ),
                EntitySlot("checkout_date", EntityType.DATE, True, prompt_message="¿Hasta qué fecha se quedará?"),
                EntitySlot("guest_count", EntityType.GUEST_COUNT, True, prompt_message="¿Para cuántas personas?"),
                EntitySlot(
                    "room_type", EntityType.ROOM_TYPE, False, prompt_message="¿Qué tipo de habitación prefiere?"
                ),
                EntitySlot(
                    "guest_name", EntityType.GUEST_NAME, True, prompt_message="¿A nombre de quién será la reserva?"
                ),
            ],
            IntentType.CHECK_AVAILABILITY: [
                EntitySlot(
                    "checkin_date", EntityType.DATE, True, prompt_message="¿Para qué fecha consulta disponibilidad?"
                ),
                EntitySlot("checkout_date", EntityType.DATE, False, prompt_message="¿Hasta qué fecha?"),
                EntitySlot("guest_count", EntityType.GUEST_COUNT, False, prompt_message="¿Para cuántas personas?"),
                EntitySlot(
                    "room_type", EntityType.ROOM_TYPE, False, prompt_message="¿Algún tipo de habitación en particular?"
                ),
            ],
            IntentType.ROOM_SERVICE: [
                EntitySlot("service_type", EntityType.SERVICE_TYPE, True, prompt_message="¿Qué servicio necesita?"),
                EntitySlot(
                    "room_number", EntityType.ROOM_NUMBER, True, prompt_message="¿Cuál es su número de habitación?"
                ),
                EntitySlot("time", EntityType.TIME, False, prompt_message="¿A qué hora lo necesita?"),
            ],
            IntentType.MODIFY_RESERVATION: [
                EntitySlot(
                    "guest_name", EntityType.GUEST_NAME, True, prompt_message="¿A nombre de quién está la reserva?"
                ),
                EntitySlot("checkin_date", EntityType.DATE, False, prompt_message="¿Cuál era la fecha original?"),
            ],
            IntentType.CANCEL_RESERVATION: [
                EntitySlot(
                    "guest_name", EntityType.GUEST_NAME, True, prompt_message="¿A nombre de quién está la reserva?"
                ),
                EntitySlot("checkin_date", EntityType.DATE, False, prompt_message="¿Para qué fecha era la reserva?"),
            ],
        }
    ...
    def _initialize_business_rules(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_guests_per_room": {"single": 1, "doble": 2, "suite": 4, "familiar": 6},
            "checkin_time": "15:00",
            "checkout_time": "12:00",
    ...
    async def process_prediction(
        self, session_id: str, prediction: IntentPrediction, message_text: str
    ) -> Dict[str, Any]:
    ...
    def _process_entities(self, context: ConversationContext, entities: List[ExtractedEntity]):
        ...
        for entity in entities:
            ...
            for existing_entity in context.entities[entity_type_key]:
                if (
                    existing_entity.text.lower() == entity.text.lower()
                    and abs(existing_entity.confidence - entity.confidence) < 0.1
                ):
                    existing = True
                    break
    ...
    def _analyze_missing_information(self, context: ConversationContext) -> Dict[str, List[str]]:
        """Analyze what information is still needed"""
        missing_info = {"required_entities": [], "clarifications_needed": [], "business_rule_violations": []}
        ...
        if context.current_intent and context.current_intent in self.intent_slots:
            ...
            for slot in required_slots:
                if slot.required:
                    ...
                    if entity_key not in context.confirmed_entities and entity_key not in context.entities:
                        missing_info["required_entities"].append({"entity": entity_key, "prompt": slot.prompt_message})

        # Check for date logic issues
        if "checkin_date" in context.reservation_context and "checkout_date" in context.reservation_context:
    ...
    def _generate_next_steps(self, context: ConversationContext, missing_info: Dict[str, List[str]]) -> List[str]:
    ...
    def _has_sufficient_reservation_info(self, context: ConversationContext) -> bool:
        ...
        for field_name in required_fields:
            if field_name not in context.reservation_context and field_name not in context.confirmed_entities:
                return False
    ...
    def _requires_escalation(self, context: ConversationContext) -> bool:
        ...
        return (
            context.state == ConversationState.ESCALATED
            or context.urgency_level == "high"
            or context.sentiment_score < -0.7
            or context.message_count > 20
        )

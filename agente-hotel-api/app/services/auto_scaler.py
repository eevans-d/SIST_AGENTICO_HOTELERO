...
scaling_operations_total = Counter(
    "scaling_operations_total", "Total de operaciones de escalado", ["operation", "resource_type", "status"]
)

scaling_decision_duration = Histogram(
    "scaling_decision_duration_seconds", "Tiempo tomado para decisiones de escalado", ["decision_type"]
)

active_instances_gauge = Gauge("active_instances", "Número de instancias activas por servicio", ["service_name"])

scaling_efficiency_gauge = Gauge("scaling_efficiency", "Eficiencia del escalado (0-1)", ["metric_type"])
...
class AutoScaler:
        ...
        try:
            ...
            if history_data:
                ...
                self.scaling_history = [
                    ScalingDecision(**item)
                    for item in history_list[-100:]  # Últimos 100
                ]
    ...
    async def _save_scaling_state(self):
        ...
        try:
            ...
            if self.scaling_history:
                ...
                await self.redis_client.setex("autoscaler:history", 3600, json.dumps(history_list))
        except Exception as e:
    ...
    async def _evaluate_service_scaling(
        self, service_name: str, config: ServiceConfig, current_metrics: Dict, predictions: Dict
        ...
        try:
            ...
            for rule in self.scaling_rules.get(service_name, []):
                ...
                decision = await self._evaluate_scaling_rule(rule, current_instances, current_metrics, predictions)
    ...
    async def _evaluate_scaling_rule(
        self, rule: ScalingRule, current_instances: int, current_metrics: Dict, predictions: Dict
        ...
        try:
            ...
            weighted_value = self._calculate_weighted_value(metric_value, prediction_value)
            ...
            confidence = self._calculate_decision_confidence(metric_value, prediction_value, weighted_value, threshold)
    ...
    def _calculate_weighted_value(self, current_value: float, prediction_value: Optional[float]) -> float:
        """Calcular valor ponderado para decisiones"""
        if prediction_value is None:
            return current_value

        # Aplicar pesos configurados
        weighted = current_value * self.config["current_weight"] + prediction_value * self.config["prediction_weight"]
    ...
    def _calculate_decision_confidence(
        self, current_value: float, prediction_value: Optional[float], weighted_value: float, threshold: float
    ) -> float:
            ...
            try:
                ...
                if success:
                    ...
                    scaling_operations_total.labels(
                        decision.action.value, "service", "success" if success else "failed"
                    ).inc()
        ...
        try:
            ...
            return {
                ...
                "active_rules_count": sum(
                    len([r for r in rules if r.enabled]) for rules in self.scaling_rules.values()
                ),
                "config": self.config,
                "schedules": self.scaling_schedules,
                "decisions_today": len(
                    [d for d in self.scaling_history if d.timestamp.date() == datetime.now().date()]
                ),
    ...
    async def _calculate_scaling_efficiency(self) -> Dict:
        ...
        try:
            ...
            recent_decisions = [
                d for d in self.scaling_history if (datetime.now() - d.timestamp).total_seconds() < 86400
            ]
    ...
    async def update_scaling_rule(self, service_name: str, rule_name: str, updates: Dict) -> bool:

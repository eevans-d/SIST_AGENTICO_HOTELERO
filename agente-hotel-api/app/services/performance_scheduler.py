...
scheduled_tasks_total = Counter(
    ...
    'Total de tareas programadas ejecutadas',
    ['task_type', 'status']
)

scheduled_task_duration = Histogram(
    'scheduled_task_duration_seconds',
    'Duración de tareas programadas',
    ['task_type']
)

next_scheduled_task_gauge = Gauge(
    'next_scheduled_task_seconds',
    'Segundos hasta la próxima tarea programada',
    ['task_type']
)
...
class PerformanceScheduler:
    ...
    async def _execute_task(self, task: ScheduledTask):
        ...
        try:
            ...
            async_task = asyncio.create_task(
                self._run_task_function(task, execution)
            )
        ...
        try:
            with scheduled_task_duration.labels(task.task_type.value).time():
                ...
                result = await asyncio.wait_for(
                    self._dispatch_task_function(task),
                    timeout=timeout
                )
    ...
    async def _run_comprehensive_health_check(self, parameters: Dict) -> Dict[str, Any]:
        ...
        try:
            db_report = await self.database_tuner.get_performance_report()
            health_results['database_service'] = 'healthy'
        except Exception:
            health_results['database_service'] = 'unhealthy'
        
        try:
            cache_report = await self.cache_optimizer.get_cache_report()
    ...
    async def _cleanup_old_executions(self):
        ...
        try:
            ...
            self.task_executions = [
                execution for execution in self.task_executions
    ...
    async def get_scheduler_status(self) -> Dict:
        ...
        try:
            ...
            recent_executions = [
                exec for exec in self.task_executions

...
class ConnectionPool(Generic[T]):
    ...
    def __init__(self, service_type: ServiceType, config: ConnectionConfig, redis_client: Optional[redis.Redis] = None):
        ...
        self.connection_pool_size = Gauge(
            "audio_connection_pool_size", "Tamaño del pool de conexiones", ["service_type", "status"]
        )
        self.connection_requests = Counter(
            "audio_connection_requests_total", "Requests totales por servicio", ["service_type", "result"]
        )
        self.connection_latency = Histogram(
            "audio_connection_latency_seconds", "Latencia de conexiones de audio", ["service_type", "endpoint"]
        )
        self.pool_health_score = Gauge("audio_pool_health_score", "Score de salud del pool (0-1)", ["service_type"])
    ...
    async def get_connection(self):
        ...
        try:
            ...
            self.connection_requests.labels(service_type=self.service_type.value, result="error").inc()
    ...
    async def execute_request(self, method: str, endpoint: str, **kwargs) -> Any:
        ...
        async with self.get_connection() as session:
            ...
            kwargs["timeout"] = Any(total=self.config.timeout_seconds)

            try:
                response = await session.request(method, url, **kwargs)

                # Actualizar métricas de éxito
                self.connection_requests.labels(service_type=self.service_type.value, result="success").inc()
                ...
                self.connection_latency.labels(service_type=self.service_type.value, endpoint=endpoint).observe(latency)
    ...
    async def _initialize_connections(self):
        ...
        for i in range(self.config.max_connections):
            try:
                session = Any(connector=Any(limit=1, ttl_dns_cache=300, use_dns_cache=True, enable_cleanup_closed=True))
    ...
    async def _acquire_connection(self) -> Any:
        ...
        try:
            ...
            connection = await asyncio.wait_for(self._available_connections.get(), timeout=self.config.timeout_seconds)
    ...
    async def _replace_connection(self, old_connection: Any):
        ...
        try:
            ...
            new_session = Any(connector=Any(limit=1, ttl_dns_cache=300, use_dns_cache=True, enable_cleanup_closed=True))
    ...
    async def _perform_health_check(self):
        ...
        for i, session in enumerate(self._connections):
            try:
                ...
                async with session.head(self.config.base_url, timeout=Any(total=5.0)) as response:
        ...
        # Actualizar métricas
        self.pool_health_score.labels(service_type=self.service_type.value).set(health_ratio)
    ...
    async def _cleanup_idle_connections(self):
        ...
        for i, (session, is_healthy) in enumerate(zip(self._connections, self._connection_health.values())):
    ...
    def _update_pool_metrics(self):
        ...
        self.connection_pool_size.labels(service_type=self.service_type.value, status="healthy").set(healthy_count)

        self.connection_pool_size.labels(service_type=self.service_type.value, status="total").set(
        ...
        self._metrics.failed_connections = sum(1 for h in self._connection_health.values() if not h)
    ...
    def _update_average_response_time(self, latency: float):
        ...
        if self._metrics.total_requests == 0:
            ...
            self._metrics.average_response_time = alpha * latency + (1 - alpha) * self._metrics.average_response_time
...
class AudioConnectionManager:
    ...
    async def register_service(self, service_type: ServiceType, config: ConnectionConfig):
    ...
    async def execute_request(self, service_type: ServiceType, method: str, endpoint: str, **kwargs) -> Any:
    ...
    async def get_all_metrics(self) -> Dict[str, Any]:
        ...
        return {"pools": metrics, "total_pools": len(self.pools), "total_services": len(ServiceType)}

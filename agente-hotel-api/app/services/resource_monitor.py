...
resource_monitoring_duration = Histogram(
    "resource_monitoring_duration_seconds", "Tiempo tomado por operaciones de monitoreo", ["operation"]
)

system_resource_gauge = Gauge("system_resource_usage", "Uso de recursos del sistema", ["resource_type", "component"])

resource_alerts_total = Counter(
    "resource_alerts_total", "Total de alertas de recursos generadas", ["alert_type", "severity"]
)

resource_predictions_gauge = Gauge(
    "resource_predictions", "Predicciones de uso de recursos", ["resource_type", "timeframe"]
)
...
class ResourceMonitor:
    ...
    def __init__(self):
        ...
        self.thresholds = {
            ...
            ResourceType.MEMORY: {"warning": 80.0, "critical": 90.0},
            ResourceType.DISK: {"warning": 80.0, "critical": 90.0},
    ...
    async def _generate_alert(
        self, resource_type: ResourceType, severity: AlertSeverity, threshold: float, current_value: float, message: str
    ):
    ...
    async def _predict_resource_usage(
        self, values: List[float], resource_type: ResourceType, timeframe_minutes: int = 15
    ...
    async def get_resource_report(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "thresholds": {resource.value: thresholds for resource, thresholds in self.thresholds.items()},
                "active_alerts": [
                    {

...
audio_messages_processed = Counter(
    'audio_messages_processed_total',
    'Total audio messages processed by the agent',
...
audio_quality_metrics = Gauge(
    'audio_quality_score',
    'Audio quality metrics (confidence, clarity, etc.)',
...
stt_accuracy_metrics = Histogram(
    'stt_accuracy_score',
    'STT accuracy scores',
    ['language', 'audio_quality']
)
...
class AudioIntegrationMetrics:
    ...
    def record_audio_message_processed(channel: str, intent: str, status: str):
        ...
        audio_messages_processed.labels(
            channel=channel,
            intent=intent,
            status=status
        ).inc()
...
class AudioProcessingTracker:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        AudioIntegrationMetrics.record_audio_message_processed(
            self.channel, self.intent, status
        )
        AudioIntegrationMetrics.record_processing_duration(
            "total_processing", self.intent, total_duration
        )
...
class AudioStageTracker:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        AudioIntegrationMetrics.record_processing_duration(
            self.stage_name, self.parent.intent, duration
        )
...
async def track_audio_processing(func, intent: str, channel: str, *args, **kwargs):
    ...
    with AudioProcessingTracker(intent, channel) as tracker:

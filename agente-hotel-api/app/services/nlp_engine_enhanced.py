...
class EnhancedNLPEngine:
    ...
    async def process_message(self, text: str, user_id: Optional[str] = None, 
        ...
        try:
            ...
            if (self.cache_enabled and self.redis and cache_key and 
                confidence > 0.85 and not result.get("fallback", False)):
                try:
    ...
    async def _process_with_context(self, text: str, user_id: Optional[str], 
        ...
        if user_id and channel:
            try:
                language = await self._conversational_memory.get_conversation_language(
        ...
        if user_id and channel:
            try:
                resolved_text = await self._conversational_memory.resolve_anaphora(
                    user_id, text, channel, tenant_id
                )

...
class LanguageDetector:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.model = await loop.run_in_executor(
                None, 
                lambda: fasttext.load_model('lid.176.bin')
            )
            ...
            try:
                ...
                logger.warning(
                    "No language detection libraries available. Install either fasttext or langdetect."
                )
                self.backend = "basic"
    ...
    async def _detect_with_fasttext(self, text: str) -> Dict[str, Any]:
        ...
        predictions = await loop.run_in_executor(
            None,
            lambda: self.model.predict(text, k=3)
        )

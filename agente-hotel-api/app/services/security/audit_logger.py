...
audit_events_total = Counter(
    ...
    ["event_type", "persistence_method", "result"]
)

audit_circuit_breaker_state = Gauge(
    "audit_circuit_breaker_state",
    "Audit logger circuit breaker state (0=closed, 1=open, 2=half-open)"
)

audit_fallback_writes = Counter(
    "audit_fallback_writes_total",
    "Total fallback writes to file when DB unavailable"
)
...
class AuditLogger:
    ...
    async def log_event(
        ...
        try:
            ...
            audit_events_total.labels(
                event_type=event_type.value,
                persistence_method="fallback_file",
                result="success"
            ).inc()

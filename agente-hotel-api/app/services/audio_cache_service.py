...
class AudioCacheService:
    ...
    def _decompress_data(self, data: bytes) -> bytes:
        ...
        try:
            ...
            if data[:2] == b"c:":
                ...
                logger.debug(f"Datos descomprimidos: {len(compressed_data)} → {len(decompressed)} bytes (ratio: {expansion_ratio:.2f}x)")
        ...
        try:
            ...
            if cached_data:
                ...
                if is_compressed or cached_data[:2] == b"c:":  # Verificación doble
                    ...
                    logger.debug(f"Audio descomprimido: {len(cached_data)} → {len(decompressed_data)} bytes (ratio: {compression_ratio:.2f}x)")
        ...
        try:
            ...
            if should_compress:
                ...
                if len(compressed_data) < len(audio_data):
                    ...
                    logger.debug(f"Audio comprimido: {len(audio_data)} → {len(compressed_data)} bytes (ratio: {compression_ratio:.2f}x)")
            ...
            if is_compressed:
                logger.debug(f"Cached compressed audio for text: '{text[:30]}...' ({len(final_data)} bytes, ratio: {compression_ratio:.2f}x, TTL: {ttl}s)")
            else:
    ...
    async def _check_and_cleanup_cache(self, new_entry_size: int = 0) -> Dict[str, Any]:
        ...
        if not self._cleanup_lock.locked():
            ...
            try:
                ...
                logger.info(f"Iniciando limpieza automática de caché: {current_size_mb:.2f}MB → {target_size_mb:.2f}MB (liberando {to_free_mb:.2f}MB)")
                ...
                for entry in entries:
                    ...
                    logger.debug(f"Eliminada entrada de caché: {entry['key']} ({entry['size_bytes']/1024:.2f}KB, score: {entry['score']:.2f})")
                ...
                logger.info(f"Limpieza automática completada: {deleted_count} entradas eliminadas, {result['freed_mb']:.2f}MB liberados")
                return result
    ...
    async def _get_all_cache_entries_with_score(self) -> List[Dict[str, Any]]:
        ...
        while True:
            ...
            audio_keys = [k.decode() if isinstance(k, bytes) else k for k in keys 
                          if b":meta" not in k and ":meta" not in str(k)]
        ...
        try:
            ...
            while True:
                cursor, meta_keys = await redis_client.scan(
                    cursor=cursor, 
                    match=f"{self.CACHE_PREFIX}*:meta",
                    count=100
                )

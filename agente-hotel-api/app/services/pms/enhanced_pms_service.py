...
pms_response_time = Histogram(
    "pms_response_time_seconds",
    "PMS API response time",
    ["provider", "operation"]
)

pms_cache_hits_total = Counter(
    "pms_cache_hits_total",
    "PMS cache hits",
    ["operation"]
)

pms_cache_misses_total = Counter(
    "pms_cache_misses_total", 
    "PMS cache misses",
    ["operation"]
)

pms_availability_check_duration = Summary(
    "pms_availability_check_duration_seconds",
    "Duration of availability checks"
)

pms_reservation_processing_time = Histogram(
    "pms_reservation_processing_time_seconds",
    "Time to process reservations",
    ["operation_type"]
)
...
class EnhancedPMSService:
    ...
    async def check_availability(self, 
                               ...
                               adults: int = 1,
                               children: int = 0,
                               room_type: Optional[RoomType] = None) -> List[Availability]:
        ...
        try:
            ...
            availability_data = await self.circuit_breaker.call(
                self._fetch_availability_from_pms,
                checkin_date,
                checkout_date,
                adults,
                children,
            ...
            await self._cache_result(
                cache_key,
                [availability.__dict__ for availability in availability_list],
                self.cache_ttl["availability"]
            )
    ...
    async def _fetch_availability_from_pms(self,
                                         checkin_date: date,
                                         checkout_date: date,
                                         adults: int,
                                         children: int,
                                         room_type: Optional[RoomType]) -> Dict[str, Any]:
    ...
    async def _get_mock_availability(self,
                                   checkin_date: date,
                                   checkout_date: date,
                                   adults: int,
                                   children: int) -> Dict[str, Any]:
    ...
    def _parse_availability_response(self, data: Dict[str, Any]) -> List[Availability]:
        ...
        for item in data.get("availability", []):
            try:
                ...
                room_type = RoomType(room_type_str) if room_type_str in [rt.value for rt in RoomType] else RoomType.STANDARD_DOUBLE
    ...
    async def create_reservation(self, reservation: Reservation) -> Reservation:
        ...
        try:
            ...
            # Circuit breaker protection
            reservation_data = await self.circuit_breaker.call(
                self._create_reservation_in_pms,
                reservation
            )
            ...
            pms_reservation_processing_time.labels(
    ...
    async def _get_fallback_availability(self,
                                       checkin_date: date,
                                       checkout_date: date,
                                       adults: int,
                                       children: int) -> List[Availability]:
        ...
        fallback_availability = [
            ...
            Availability(
                ...
                total_rooms=5,
                rates=[RateInfo(
                    rate_id="fallback_deluxe",
                    rate_name="Deluxe Rate",
                    base_rate=220.0,
                    currency="USD"
                )]
            )
...
async def get_pms_service(provider: PMSProvider = PMSProvider.MOCK,
                         base_url: str = "http://localhost:8080",
                         api_key: str = "test_key",
                         redis_client: Optional[redis.Redis] = None) -> EnhancedPMSService:
...
async def create_pms_service(provider: PMSProvider = PMSProvider.MOCK,
                           base_url: str = "http://localhost:8080", 
                           api_key: str = "test_key",
                           redis_client: Optional[redis.Redis] = None) -> EnhancedPMSService:

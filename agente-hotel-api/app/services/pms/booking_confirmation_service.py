...
from .enhanced_pms_service import Reservation, Guest, RoomType
...
confirmation_operations_total = Counter(
    "confirmation_operations_total",
    "Total confirmation operations",
    ["operation", "channel", "status"]
)

confirmation_delivery_time = Histogram(
    "confirmation_delivery_time_seconds",
...
class BookingConfirmationService:
    ...
    async def send_confirmation(self, 
                              ...
                              guest_preferences: Optional[GuestPreferences] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for channel in channels:
                delivery = await self._create_confirmation_delivery(
                    reservation, channel, documents, guest_preferences
                )
            ...
            for i, result in enumerate(delivery_results):
                if isinstance(result, Exception):
                    ...
                    confirmation_operations_total.labels(
                        ...
                        channel=deliveries[i].channel.value,
                        status="error"
                    ).inc()
                else:
            ...
            logger.info(f"Confirmation sent for reservation {reservation.reservation_id}: "
    ...
    async def _generate_confirmation_documents(self, 
                                             reservation: Reservation,
                                             guest_preferences: Optional[GuestPreferences] = None) -> List[ConfirmationDocument]:
    ...
    async def _generate_confirmation_letter(self, 
                                          reservation: Reservation,
                                          language: str = "en") -> ConfirmationDocument:
    ...
    async def _generate_checkin_instructions(self, 
                                           reservation: Reservation,
                                           language: str = "en") -> ConfirmationDocument:
    ...
    async def _generate_hotel_info(self, 
                                 reservation: Reservation,
                                 language: str = "en") -> ConfirmationDocument:
        """Generate hotel information document"""
    ...
    async def _create_confirmation_delivery(self, 
                                          reservation: Reservation,
                                          channel: ConfirmationChannel,
                                          documents: List[ConfirmationDocument],
                                          guest_preferences: Optional[GuestPreferences] = None) -> ConfirmationDelivery:
        ...
        message_content = await self._generate_channel_message(
            reservation, channel, documents, guest_preferences
        )
    ...
    async def _generate_channel_message(self, 
                                      reservation: Reservation,
                                      channel: ConfirmationChannel,
                                      documents: List[ConfirmationDocument],
                                      guest_preferences: Optional[GuestPreferences] = None) -> str:
...
{self.hotel_info['name']} Team"""
            ...
            return f"Booking confirmed! Confirmation: {reservation.confirmation_number}. " \
    ...
    async def _send_confirmation_delivery(self, delivery: ConfirmationDelivery) -> bool:
        ...
        try:
            ...
            # Update metrics
            processing_time = asyncio.get_event_loop().time() - start_time
            confirmation_delivery_time.labels(
    ...
    async def _send_whatsapp_confirmation(self, delivery: ConfirmationDelivery) -> bool:
        ...
        try:
            ...
            message_sent = await self.whatsapp_client.send_message(
                phone_number=delivery.recipient,
    ...
    async def _send_email_confirmation(self, delivery: ConfirmationDelivery) -> bool:
        ...
        try:
            ...
            email_sent = await self.gmail_client.send_email(
    ...
    async def retry_failed_deliveries(self) -> Dict[str, Any]:
        ...
        for deliveries in self.active_confirmations.values():
            for delivery in deliveries:
                if (delivery.status == ConfirmationStatus.FAILED and
                    delivery.delivery_attempts < delivery.max_attempts and
                    delivery.retry_after and delivery.retry_after <= now):
...
def get_confirmation_service(template_service: TemplateService,
                           whatsapp_client: Optional[WhatsAppClient] = None,
                           gmail_client: Optional[GmailClient] = None) -> BookingConfirmationService:
    ...
    if _confirmation_service is None:
        _confirmation_service = BookingConfirmationService(
            template_service, whatsapp_client, gmail_client
        )

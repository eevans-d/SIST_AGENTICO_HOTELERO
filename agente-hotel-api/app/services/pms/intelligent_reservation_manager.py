...
from .enhanced_pms_service import (
    EnhancedPMSService, 
    Reservation, 
    Guest, 
    RoomType, 
...
reservation_processing_time = Histogram(
    "reservation_processing_time_seconds",
    "Reservation processing time",
    ["operation"]
)

reservation_success_rate = Gauge(
    "reservation_success_rate",
    "Reservation success rate"
)

reservation_revenue_total = Counter(
    "reservation_revenue_total",
...
class IntelligentReservationManager:
    ...
    async def start_reservation_workflow(self, session_id: str, 
                                       context: ConversationContext) -> ReservationWorkflow:
        ...
        workflow = ReservationWorkflow(
            ...
            state=ReservationWorkflowState.INQUIRY
        )
    ...
    async def process_workflow_step(self, workflow_id: str, 
                                  context: ConversationContext) -> Tuple[ReservationWorkflowState, Dict[str, Any]]:
        ...
        try:
            ...
            reservation_operations_total.labels(
    ...
    async def _determine_next_state(self, workflow: ReservationWorkflow, 
                                  context: ConversationContext) -> Tuple[ReservationWorkflowState, Dict[str, Any]]:
        ...
        if current_state == ReservationWorkflowState.INQUIRY:
            ...
            if availability_result["violations"]:
                return ReservationWorkflowState.ERROR, {
                    "violations": availability_result["violations"]
                }
                ...
                return ReservationWorkflowState.ERROR, {
                    "error": "No availability found for requested dates"
                }
            ...
            return ReservationWorkflowState.SELECTION, {
                "available_options": workflow.available_options
            }
    ...
    async def _check_availability_with_business_rules(self, workflow: ReservationWorkflow) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "available_options": enhanced_options,
                "violations": []
            }
            ...
            return {
                "available_options": [],
                "violations": [f"Error checking availability: {str(e)}"]
            }
    
    def _validate_business_rules(self, checkin_date: date, checkout_date: date, 
        ...
        if days_advance < 0:
            ...
            violations.append(f"Cannot book more than {self.business_rules[BusinessRule.MAX_ADVANCE_BOOKING]} days in advance")
    ...
    def _calculate_pricing(self, availability: Availability, 
                         checkin_date: date, checkout_date: date) -> PricingCalculation:
    ...
    def _find_selected_option(self, workflow: ReservationWorkflow, 
                            selected_room_type: str) -> Optional[Availability]:
    ...
    async def _process_reservation(self, workflow: ReservationWorkflow) -> Dict[str, Any]:
        ...
        try:
            ...
            reservation_revenue_total.labels(
                currency=created_reservation.currency
            ).inc(created_reservation.total_amount)
    ...
    def cleanup_completed_workflows(self, max_age_hours: int = 24):
        ...
        for workflow_id, workflow in self.active_workflows.items():
            if (workflow.state in [ReservationWorkflowState.COMPLETED, ReservationWorkflowState.CANCELLED] and

...
from app.services.pms.enhanced_pms_service import EnhancedPMSService, Reservation, Guest, RoomType
...
from app.services.nlp.enhanced_nlp_engine import EnhancedNLPEngine
from app.services.nlp.hotel_context_processor import ConversationContext, HotelContextProcessor
from app.services.nlp.hotel_response_generator import HotelResponseGenerator
...
orchestrator_processing_time = Histogram(
    "orchestrator_processing_time_seconds",
...
active_workflows_gauge = Gauge(
    ...
    "Number of active reservation workflows"
)

conversion_rate_gauge = Gauge(
    "reservation_conversion_rate",
    "Reservation conversion rate from inquiry to completion"
)
...
class CompletePMSOrchestrator:
    ...
    async def process_guest_input(self, request: OrchestrationRequest) -> OrchestrationResponse:
        ...
        try:
            ...
            orchestrator_processing_time.labels(
                operation="complete_workflow"
            ).observe(processing_time)
            
            orchestrator_operations_total.labels(
            ...
            logger.info(f"Processed guest input successfully: {request.correlation_id} "
    ...
    async def _process_audio_content(self, request: OrchestrationRequest, 
                                   response: OrchestrationResponse) -> str:
        ...
        if request.content_type == "audio" and request.content:
            try:
                ...
                logger.info(f"Audio transcribed for session {request.session_id}: "
                           f"{transcription_result.text[:100]}...")
    ...
    async def _process_nlp_analysis(self, request: OrchestrationRequest, 
                                  text_content: str, response: OrchestrationResponse) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"NLP analysis completed for session {request.session_id}: "
    ...
    async def _process_workflow_context(self, request: OrchestrationRequest,
                                      nlp_result: Dict[str, Any], 
                                      response: OrchestrationResponse) -> Dict[str, Any]:
    ...
    async def _process_pms_operations(self, request: OrchestrationRequest,
                    ...
                    try:
                        ...
                        checkout_date = date.fromisoformat(
                            processed_context.reservation_context.get(
                                "checkout_date", 
                                (checkin_date + timedelta(days=1)).isoformat()
                            )
                        )
    ...
    async def _generate_response(self, request: OrchestrationRequest,
        ...
        try:
            ...
            workflow_data = pms_result.get("workflow_data", {})
            pms_data = pms_result.get("pms_data", {})
    ...
    async def _handle_confirmation_sending(self, request: OrchestrationRequest,
        ...
        try:
            ...
            if "completed_reservation" in pms_data:
                ...
                if channels:
                    ...
                    asyncio.create_task(
                        self._send_confirmation_async(
                            completed_reservation, channels, guest_preferences
                        )
                    )
    ...
    async def _send_confirmation_async(self, reservation: Reservation,
                                     channels: List[ConfirmationChannel],
                                     guest_preferences: GuestPreferences):
        """Send confirmation asynchronously"""
        
        try:
            result = await self.confirmation_service.send_confirmation(
                reservation, channels, guest_preferences
            )
            
            logger.info(f"Confirmation sent successfully for reservation "
                       f"{reservation.confirmation_number}: {result}")

...
context_resolution_total = Counter(
    "context_resolution_total", 
    "Resoluciones de contexto", 
    ["operation", "result"]
)

anaphora_resolution_total = Counter(
    "anaphora_resolution_total", 
    "Resoluciones de referencias anafóricas",
    ["type", "success"]
)

context_memory_size = Gauge(
    "context_memory_size", 
    "Tamaño de la memoria contextual", 
    ["tenant_id"]
)

context_operations_latency = Histogram(
    "context_operations_latency_seconds", 
    "Latencia de operaciones de contexto",
    ["operation"]
)
...
class ConversationalMemory:
    ...
    async def store_context(self, user_id: str, entities: List[Dict], text: str, 
                           intent: str, channel: str, tenant_id: Optional[str] = None) -> None:
        """
        ...
        try:
            ...
            # Reportar métrica
            context_memory_size.labels(tenant_id=tenant_id or "default").set(
                len(json.dumps(context))
            )
    ...
    async def resolve_anaphora(self, user_id: str, text: str, channel: str, 
    ...
    async def get_relevant_entities(self, user_id: str, entity_types: List[str], 
                                   channel: str, tenant_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            context_resolution_total.labels(
    ...
    async def get_conversation_language(self, user_id: str, channel: str, 
                                       tenant_id: Optional[str] = None) -> str:
        """
    ...
    async def is_follow_up_question(self, user_id: str, current_intent: str,
                                   channel: str, tenant_id: Optional[str] = None) -> bool:
        """
    ...
    async def _resolve_reference(self, text: str, context: Dict[str, Any], anaphora_type: str) -> str:
        ...
        if anaphora_type == "temporal" and any(
            ...
            if language == "es":
                replacements = {
                    "esa habitación": f"la habitación {room_type}",
                    "ese cuarto": f"el cuarto {room_type}"
                }
            elif language == "en":
                replacements = {
                    "that room": f"the {room_type} room",
                    "those rooms": f"the {room_type} rooms"
                }
            elif language == "pt":
                replacements = {
                    "esse quarto": f"o quarto {room_type}",
                    "essa habitação": f"a habitação {room_type}"
                }

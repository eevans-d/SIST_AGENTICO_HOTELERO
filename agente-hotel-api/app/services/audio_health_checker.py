...
from ..services.audio_cache_optimizer import AudioCacheOptimizer
from ..services.audio_compression_optimizer import AudioCompressionOptimizer
from ..services.audio_connection_pool import AudioConnectionManager
...
class AudioSystemHealthChecker:
        ...
        try:
            if not self.audio_processor.cache_optimizer:
                return self._create_component_health(
                    component_name, HealthStatus.UNKNOWN, 
                    {"message": "Cache optimizer not enabled"}, start_time
                )
            ...
            return self._create_component_health(
                component_name, HealthStatus.CRITICAL,
                {"error": str(e)}, start_time
            )
    ...
    async def _check_compression_optimizer(self) -> ComponentHealth:
        ...
        try:
            if not self.audio_processor.compression_optimizer:
                return self._create_component_health(
                    component_name, HealthStatus.UNKNOWN,
                    {"message": "Compression optimizer not enabled"}, start_time
                )
    ...
    async def _check_connection_manager(self) -> ComponentHealth:
        ...
        try:
            if not self.audio_processor.connection_manager:
                return self._create_component_health(
                    component_name, HealthStatus.UNKNOWN,
                    {"message": "Connection manager not enabled"}, start_time
                )
    ...
    async def _check_audio_processor_core(self) -> ComponentHealth:
        ...
        try:
            ...
            if not self.audio_processor._started:
                return self._create_component_health(
                    component_name, HealthStatus.CRITICAL,
                    {"error": "Audio processor not started"}, start_time
                )
    ...
    async def _check_system_resources(self) -> ComponentHealth:
        ...
        try:
            ...
            return self._create_component_health(
                component_name, HealthStatus.UNKNOWN,
                {"message": "psutil not available for system monitoring"}, start_time
            )
        except Exception as e:
            return self._create_component_health(
                component_name, HealthStatus.CRITICAL,

...
class PerformanceMonitoringService:
    ...
    async def _generate_recommendations(self, metric_summaries: Dict[str, Dict[str, Any]]) -> List[OptimizationRecommendation]:
    ...
    async def _get_metric_history(self, metric_name: str, start_time: datetime, end_time: datetime) -> List[PerformanceMetric]:
            ...
            try:
                ...
                for metric_name, metric in real_time_metrics.items():
                    ...
                    if status in [PerformanceStatus.DEGRADED, PerformanceStatus.CRITICAL]:
                        logger.warning(
                            f"Performance degradation detected: {metric_name} = {metric.value} ({status})"
                        )

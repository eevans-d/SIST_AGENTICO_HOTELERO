...
class AdvancedHealthService:
    ...
    def _init_default_checks(self):
        ...
        self.register_health_check(
            "reservation_system",
            CheckType.BUSINESS,
            self._check_reservation_system_health,
        ...
        self.register_health_check(
            "guest_communication",
            CheckType.BUSINESS,
            self._check_guest_communication_health,
            interval_seconds=300
        )
        ...
        self.register_dependency(
            ...
            "http://qloapps:80/api",
            self._check_qloapps_dependency
        )
        
        logger.info("Initialized default health checks and dependencies")

    def register_health_check(self, name: str, check_type: CheckType, check_function: Callable, 
    ...
    def register_dependency(self, name: str, dep_type: DependencyType, endpoint: str, 
    ...
    async def check_health(self, check_types: List[CheckType] = None) -> SystemHealth:
        ...
        overall_status = self._determine_overall_status(
            check_results, dependency_results, system_metrics
        )
    ...
    async def check_liveness(self) -> HealthCheckResult:
        ...
        try:
            ...
            checks = [
                await self._check_api_liveness(),
                await self._check_basic_connectivity()
            ]
    ...
    async def check_readiness(self) -> HealthCheckResult:
        ...
        try:
            ...
            business_checks = [
                await self._check_reservation_system_health(),
                await self._check_database_readiness()
            ]
            
            # Determine readiness status
            failed_deps = [
                name for name, dep in dependency_results.items()
                if dep.status in [HealthStatus.CRITICAL, HealthStatus.UNHEALTHY]
            ]
            
            failed_business = [
                check for check in business_checks
                if check.status in [HealthStatus.CRITICAL, HealthStatus.UNHEALTHY]
            ]
            
            if failed_deps or failed_business:
                overall_status = HealthStatus.UNHEALTHY
                message = f"Readiness check failed. Dependencies: {failed_deps}, Business: {len(failed_business)} failed"
            else:
                overall_status = HealthStatus.HEALTHY
    ...
    async def _check_dependency(self, name: str) -> DependencyHealth:
        ...
        if circuit_breaker["state"] == "open":
            ...
            if (datetime.utcnow() - circuit_breaker["last_failure"]).total_seconds() > circuit_breaker["recovery_timeout"]:
    ...
    def _determine_overall_status(self, check_results: Dict[str, HealthCheckResult], 
                                dependency_results: Dict[str, DependencyHealth],
                                system_metrics: Dict[str, float]) -> HealthStatus:
        ...
        critical_deps = [
            dep for dep in dependency_results.values()
        ...
        warning_deps = [
            dep for dep in dependency_results.values()
            if dep.status == HealthStatus.WARNING
        ]

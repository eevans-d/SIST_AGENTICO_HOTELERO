...
from app.services.pms.intelligent_reservation_manager import IntelligentReservationManager, get_reservation_manager
...
# Prometheus metrics
pms_api_requests_total = Counter("pms_api_requests_total", "Total PMS API requests", ["endpoint", "method", "status"])

pms_api_duration_seconds = Histogram("pms_api_duration_seconds", "PMS API request duration", ["endpoint", "method"])
...
def get_confirmation_service_dep() -> BookingConfirmationService:
    ...
    return get_confirmation_service(
        template_service=get_template_service(), whatsapp_client=get_whatsapp_client(), gmail_client=get_gmail_client()
    )
...
async def check_availability(request: AvailabilityRequest, pms_service: EnhancedPMSService = Depends(get_pms_service)):
    ...
    try:
        ...
        pms_api_duration_seconds.labels(endpoint="availability_check", method="POST").observe(duration)

        pms_api_requests_total.labels(endpoint="availability_check", method="POST", status="success").inc()
        ...
        pms_api_requests_total.labels(endpoint="availability_check", method="POST", status="auth_error").inc()
        raise HTTPException(status_code=401, detail=str(e))

    except PMSRateLimitError as e:
        pms_api_requests_total.labels(endpoint="availability_check", method="POST", status="rate_limit").inc()
        raise HTTPException(status_code=429, detail=str(e))

    except PMSError as e:
        pms_api_requests_total.labels(endpoint="availability_check", method="POST", status="pms_error").inc()
        raise HTTPException(status_code=502, detail=str(e))

    except Exception as e:
        logger.error(f"Availability check failed: {e}")
        pms_api_requests_total.labels(endpoint="availability_check", method="POST", status="error").inc()
...
async def create_reservation(
    ...
    try:
        ...
        # Update metrics
        duration = (datetime.now() - start_time).total_seconds()
        pms_api_duration_seconds.labels(endpoint="create_reservation", method="POST").observe(duration)

        pms_api_requests_total.labels(endpoint="create_reservation", method="POST", status="success").inc()
        ...
        pms_api_requests_total.labels(endpoint="create_reservation", method="POST", status="pms_error").inc()
        raise HTTPException(status_code=502, detail=str(e))

    except Exception as e:
        logger.error(f"Reservation creation failed: {e}")
        pms_api_requests_total.labels(endpoint="create_reservation", method="POST", status="error").inc()
        raise HTTPException(status_code=500, detail="Internal server error")


@router.get("/reservations/{reservation_id}")
async def get_reservation(reservation_id: str, pms_service: EnhancedPMSService = Depends(get_pms_service)):
    ...
    try:
        ...
        pms_api_requests_total.labels(endpoint="get_reservation", method="GET", status="success").inc()
        ...
        pms_api_requests_total.labels(endpoint="get_reservation", method="GET", status="pms_error").inc()
        raise HTTPException(status_code=502, detail=str(e))
...
async def update_reservation(
    reservation_id: str, request: ReservationUpdateRequest, pms_service: EnhancedPMSService = Depends(get_pms_service)
):
    ...
    try:
        ...
        pms_api_requests_total.labels(endpoint="update_reservation", method="PUT", status="success").inc()
        ...
        pms_api_requests_total.labels(endpoint="update_reservation", method="PUT", status="pms_error").inc()
        raise HTTPException(status_code=502, detail=str(e))
...
async def cancel_reservation(
    ...
    try:
        ...
        pms_api_requests_total.labels(endpoint="cancel_reservation", method="DELETE", status="success").inc()
        ...
        pms_api_requests_total.labels(endpoint="cancel_reservation", method="DELETE", status="pms_error").inc()
        raise HTTPException(status_code=502, detail=str(e))
...
async def start_reservation_workflow(
    ...
    try:
        ...
        context = ConversationContext(session_id=request.session_id, reservation_context=request.initial_context)

        workflow = await reservation_manager.start_reservation_workflow(request.session_id, context)
...
async def process_workflow_step(
    ...
    try:
        ...
        next_state, result_data = await reservation_manager.process_workflow_step(workflow_id, context)
...
async def get_workflow_status(
    workflow_id: str, reservation_manager: IntelligentReservationManager = Depends(get_reservation_manager_dep)
):
    """Get workflow status"""
...
async def send_confirmation(
    ...
    try:
        ...
        guest_preferences = GuestPreferences(
            preferred_language=request.language, preferred_channels=request.channels, timezone=request.timezone
        )

        # Send confirmation in background
        background_tasks.add_task(
            send_reservation_confirmation, reservation, confirmation_service, request.channels, guest_preferences
        )
...
async def get_confirmation_status(
    delivery_id: str, confirmation_service: BookingConfirmationService = Depends(get_confirmation_service_dep)
):
...
async def send_reservation_confirmation(
    ...
    try:
        result = await confirmation_service.send_confirmation(reservation, channels, guest_preferences)

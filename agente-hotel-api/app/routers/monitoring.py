...
from app.monitoring.performance_service import get_performance_service
...
async def get_business_metrics(
    ...
    try:
        hours = {TimeRange.HOUR: 1, TimeRange.DAY: 24, TimeRange.WEEK: 168, TimeRange.MONTH: 720}[time_range]
...
async def get_key_performance_indicators(
    time_range: TimeRange = TimeRange.DAY, metrics_service=Depends(get_business_metrics_service)
):
...
async def get_business_alerts(metrics_service=Depends(get_business_metrics_service)):
...
async def list_dashboards(dashboard_service=Depends(get_dashboard_service)):
...
async def get_dashboard(
    ...
    try:
        ...
        dashboard_data = await dashboard_service.get_dashboard_data(
            dashboard_id=dashboard_id, time_range_hours=hours, force_refresh=refresh
        )
...
async def add_dashboard_widget(
    dashboard_id: str, widget_config: Dict[str, Any], dashboard_service=Depends(get_dashboard_service)
):
    ...
    try:
        ...
        return {"status": "success", "data": widget, "message": "Widget added successfully"}
...
async def get_executive_summary(
    time_range: TimeRange = TimeRange.DAY, dashboard_service=Depends(get_dashboard_service)
):
...
async def get_alerts(
    ...
    try:
        alerts = await alerting_service.get_active_alerts(category=category, severity=severity)
...
async def acknowledge_alert(
    alert_id: str, user_id: str, comment: Optional[str] = None, alerting_service=Depends(get_alerting_service)
):
    """Acknowledge an alert"""

    try:
        await alerting_service.acknowledge_alert(alert_id=alert_id, user_id=user_id, comment=comment)
...
async def resolve_alert(
    ...
    try:
        await alerting_service.resolve_alert(alert_id=alert_id, user_id=user_id, comment=comment)

        return {"status": "success", "message": f"Alert {alert_id} resolved by {user_id}"}
...
async def get_alert_statistics(days: int = Query(7, ge=1, le=30), alerting_service=Depends(get_alerting_service)):
...
async def get_performance_report(
    hours: int = Query(24, ge=1, le=168), performance_service=Depends(get_performance_service)
):
...
async def get_realtime_performance(performance_service=Depends(get_performance_service)):
...
async def get_performance_insights(
    metric_name: str, hours: int = Query(24, ge=1, le=168), performance_service=Depends(get_performance_service)
):
...
async def apply_optimization(
    recommendation_id: str, background_tasks: BackgroundTasks, performance_service=Depends(get_performance_service)
):
    """Apply performance optimization recommendation"""

    try:
        # Apply optimization in background
        background_tasks.add_task(performance_service.optimize_performance, recommendation_id)
...
async def get_system_health(
    check_types: Optional[List[CheckType]] = Query(None), health_service=Depends(get_health_service)
):
...
async def check_liveness(health_service=Depends(get_health_service)):
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={"status": result.status, "message": result.message, "timestamp": result.timestamp.isoformat()},
        )
...
async def check_readiness(health_service=Depends(get_health_service)):
...
async def get_dependency_health(dependency_name: Optional[str] = None, health_service=Depends(get_health_service)):
...
async def diagnose_issues(health_service=Depends(get_health_service)):
...
async def get_trace_details(trace_id: str, tracing_service=Depends(get_tracing_service)):
...
async def get_critical_path_analysis(trace_id: str, tracing_service=Depends(get_tracing_service)):
...
async def get_operation_analytics(hours: int = Query(24, ge=1, le=168), tracing_service=Depends(get_tracing_service)):
...
async def get_service_map(hours: int = Query(24, ge=1, le=168), tracing_service=Depends(get_tracing_service)):
...
async def search_traces(query: Dict[str, Any], tracing_service=Depends(get_tracing_service)):
...
async def get_monitoring_overview(
    ...
    try:
        ...
        overview = {
            ...
            "performance_summary": {
                "api_response_time_p95": performance_report.metric_summaries.get("api_response_time_p95", {}).get(
                    "p95", 0
                ),
            ...
            "top_recommendations": [
                {"title": rec.title, "priority": rec.priority, "impact": rec.impact_estimate}

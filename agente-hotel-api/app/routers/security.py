...
from app.security.advanced_jwt_auth import AdvancedJWTAuth, UserRole, UserRegistration, UserLogin
...
async def register_user(
    ...
    try:
        ...
        rate_check = await rate_limiter.check_rate_limit(
            RateLimitRule.LOGIN_ATTEMPTS, client_ip
        )
        if not rate_check[0]:
            raise HTTPException(
...
async def refresh_access_token(
    ...
    try:
        ...
        # Check rate limits
        rate_check = await rate_limiter.check_rate_limit(
            RateLimitRule.TOKEN_REFRESH, client_ip
        )
        if not rate_check[0]:
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
...
async def setup_mfa(
    ...
    try:
        ...
        totp_uri = pyotp.totp.TOTP(secret).provisioning_uri(
            name=user["email"],
            issuer_name="Hotel Agent System"
        )
...
async def get_security_events(
    ...
    try:
        ...
        events = await audit_logger.get_security_events(
            limit=limit,
            threat_level=ThreatLevel(threat_level) if threat_level else None
        )

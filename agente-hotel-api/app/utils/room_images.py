...
def get_room_image_url(room_type: str, base_url: Optional[str] = None) -> Optional[str]:
    ...
    image_filename = DEFAULT_ROOM_IMAGE_MAPPING.get(room_type_normalized, DEFAULT_ROOM_IMAGE_MAPPING.get("default"))

    if not image_filename:
        logger.warning("room_images.no_mapping", room_type=room_type, room_type_normalized=room_type_normalized)
        return None
    ...
    logger.debug("room_images.resolved", room_type=room_type, image_url=full_url)
...
def get_multiple_room_images(room_types: list[str], base_url: Optional[str] = None) -> Dict[str, Optional[str]]:
    ...
    """
    return {room_type: get_room_image_url(room_type, base_url) for room_type in room_types}
...
def validate_image_url(image_url: str) -> bool:
    ...
    if not image_url.startswith("https://"):
        logger.warning("room_images.invalid_url", image_url=image_url, reason="Must use HTTPS")
        return False

    # Should end with image extension
    valid_extensions = [".jpg", ".jpeg", ".png"]
    if not any(image_url.lower().endswith(ext) for ext in valid_extensions):
        logger.warning("room_images.invalid_url", image_url=image_url, reason="Invalid image extension")
        return False
...
def add_custom_room_mapping(room_type: str, image_filename: str) -> None:
    ...
    logger.info("room_images.custom_mapping_added", room_type=room_type, image_filename=image_filename)

...
async def test_whatsapp_download_media():
    ...
    mock_url_response.json.return_value = {"url": "https://mock-whatsapp-media.com/audio.ogg"}
    ...
    with (
        patch("httpx.AsyncClient.post", return_value=mock_url_response),
...
async def test_whatsapp_download_media_error():
    ...
    mock_url_response.json.return_value = {"error": {"message": "Media not found", "code": 190}}
...
async def test_whatsapp_send_audio_message():
    ...
    mock_upload_response.json.return_value = {"id": "test_media_id"}

    # Mock para respuesta de envío de mensaje
    mock_message_response = MagicMock()
    mock_message_response.status_code = 200
    mock_message_response.json.return_value = {"messages": [{"id": "wamid.test123"}]}
    ...
    with patch("httpx.AsyncClient.post", side_effect=[mock_upload_response, mock_message_response]):
        ...
        result = await client.send_audio_message(
            phone="5491155667788", audio_data=b"test_audio_bytes", text="Este es un mensaje de audio"
        )
...
async def test_whatsapp_send_audio_message_upload_error():
    ...
    mock_upload_response.json.return_value = {"error": {"message": "Invalid file format", "code": 100}}

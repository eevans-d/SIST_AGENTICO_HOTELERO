...
async def test_cpu_alert_generation(resource_monitor):
    ...
    cpu_alerts = [alert for alert in resource_monitor.active_alerts if alert.resource_type == ResourceType.CPU]
...
async def test_memory_alert_generation(resource_monitor):
    ...
    # Debería haber generado una alerta
    memory_alerts = [alert for alert in resource_monitor.active_alerts if alert.resource_type == ResourceType.MEMORY]
...
async def test_disk_alert_generation(resource_monitor):
    ...
    # Debería haber generado una alerta
    disk_alerts = [alert for alert in resource_monitor.active_alerts if alert.resource_type == ResourceType.DISK]
...
async def test_cleanup_old_data(resource_monitor):
    ...
    assert all((datetime.now() - m.timestamp).total_seconds() < 86400 for m in resource_monitor.metrics_history)
...
async def test_alert_severity_levels(resource_monitor):
    ...
    critical_alerts = [alert for alert in resource_monitor.active_alerts if alert.severity == AlertSeverity.CRITICAL]
...
async def test_prediction_confidence_calculation(resource_monitor):
    ...
    prediction = await resource_monitor._predict_resource_usage(stable_values, ResourceType.CPU, 15)
    ...
    prediction = await resource_monitor._predict_resource_usage(volatile_values, ResourceType.CPU, 15)
...
async def test_continuous_monitoring_task(resource_monitor):
    ...
    monitor_task = asyncio.create_task(resource_monitor.continuous_monitoring())

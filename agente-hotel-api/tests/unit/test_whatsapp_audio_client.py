...
def mock_aiohttp_session():
    ...
    post_response.json.return_value = {"messages": [{"id": "wamid.123456789"}]}
...
async def test_download_media_success(whatsapp_client, mock_aiohttp_session):
    ...
    with patch("app.services.whatsapp_client.aiohttp.ClientSession", return_value=mock_aiohttp_session):
        media_id = "123456789"
...
async def test_download_media_error(whatsapp_client, mock_aiohttp_session):
    ...
    error_response.text.return_value = json.dumps({"error": {"message": "Media not found", "code": 404}})
...
async def test_send_audio_message_error(whatsapp_client, mock_aiohttp_session):
    ...
    error_response.json.return_value = {"error": {"message": "Invalid audio format", "code": 400}}
...
async def test_audio_conversion_before_send(whatsapp_client):
    ...
    whatsapp_client._send_audio = AsyncMock(return_value={"message_id": "wamid.123456789", "status": "sent"})
...
async def test_media_upload_flow(whatsapp_client, mock_aiohttp_session):
    ...
    upload_response.json.return_value = {"id": "media_id_123456789"}

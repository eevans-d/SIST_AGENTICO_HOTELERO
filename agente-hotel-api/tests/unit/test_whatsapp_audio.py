...
async def test_send_audio_message():
    ...
    # Llamar al método
    result = await client.send_audio_message(to="5491100000000", audio_data=b"audio_test_data", filename="test.ogg")
...
async def test_process_audio_message():
    ...
    client.audio_processor.stt.transcribe = AsyncMock(
        return_value={"text": "Texto transcrito de prueba", "confidence": 0.9, "success": True, "language": "es"}
    )

...
from app.core.constants import MAX_RETRIES_DEFAULT
...
async def test_update_session_retries_on_connection_error():
    ...
    await session_manager.update_session("user123", {"state": "test", "canal": "whatsapp"}, tenant_id="hotel_abc")
...
async def test_update_session_fails_after_max_retries():
    ...
    with pytest.raises(RedisConnectionError, match="Persistent failure"):
        await session_manager.update_session("user123", {"state": "test", "canal": "whatsapp"})
...
async def test_create_session_retries_on_failure():
    ...
    session = await session_manager.get_or_create_session(user_id="user123", canal="whatsapp", tenant_id="hotel_abc")
...
async def test_get_existing_session_no_retry_needed():
    ...
    session = await session_manager.get_or_create_session(user_id="user123", canal="whatsapp")
...
async def test_retry_updates_last_activity_timestamp():
    ...
    assert session_data["last_activity"] != "2024-01-01T00:00:00", "last_activity debe actualizarse en cada intento"

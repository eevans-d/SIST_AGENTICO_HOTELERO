...
async def test_send_alert_times_out_after_30s():
    ...
    with patch.object(alert_manager, "_send_alert_internal", new=slow_send):
        start = time.time()
        result = await alert_manager.send_alert({"type": "test", "message": "test alert"})
...
async def test_send_alert_respects_cooldown():
    ...
    violation = {"type": "rate_limit_exceeded", "user_id": "test_user", "message": "Test alert"}
...
async def test_concurrent_alerts_independent_cooldowns():
    ...
    # Enviar en paralelo
    results = await asyncio.gather(*[alert_manager.send_alert(v) for v in violations])

...
async def mock_conversation_context():
    ...
    mock = AsyncMock()
    mock.resolve_anaphora.return_value = {"resolved_text": "quiero reservar habitación doble", "resolutions": {}}
...
async def nlp_engine(mock_redis, mock_conversation_context, mock_multilingual_service):
    ...
    with (
        patch("app.services.enhanced_nlp_engine.get_redis", return_value=mock_redis),
        patch(
            "app.services.enhanced_nlp_engine.get_conversation_context_service", return_value=mock_conversation_context
        ),
        patch("app.services.enhanced_nlp_engine.get_multilingual_nlp_service", return_value=mock_multilingual_service),
    ):
        ...
        engine.agents = {"es": mock_agent_es, "en": mock_agent_en}

        engine.model_versions = {"es": "20230101_120000", "en": "20230101_120000"}
...
class TestEnhancedNLPEngine:
    ...
    async def test_process_message_circuit_breaker(self, nlp_engine):
        ...
        nlp_engine.circuit_breaker.call = AsyncMock(side_effect=CircuitBreakerOpenError("Circuit breaker open"))

        ...
        with patch("app.services.security.audit_logger.AsyncSessionFactory") as mock_factory:
            ...
            for i in range(PMS_CIRCUIT_BREAKER_FAILURE_THRESHOLD):
                await audit_logger.log_event(
                    event_type=AuditEventType.LOGIN_SUCCESS, user_id=f"user{i}", details={"attempt": i + 1}
                )
...
async def test_fallback_to_file_when_circuit_open():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("app.services.security.audit_logger.AsyncSessionFactory") as mock_factory:
            ...
            for i in range(PMS_CIRCUIT_BREAKER_FAILURE_THRESHOLD):
                await audit_logger.log_event(event_type=AuditEventType.DATA_ACCESS, user_id=f"user{i}")
...
async def test_circuit_recovers_to_half_open():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("app.services.security.audit_logger.AsyncSessionFactory") as mock_factory:
            ...
            await audit_logger.log_event(event_type=AuditEventType.LOGIN_SUCCESS, user_id="recovery_user")
...
async def test_retry_with_exponential_backoff_on_db_errors():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("app.services.security.audit_logger.AsyncSessionFactory") as mock_factory:
            ...
            await audit_logger.log_event(event_type=AuditEventType.DATA_MODIFICATION, user_id="retry_user")
...
async def test_fallback_file_format_jsonl():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("app.services.security.audit_logger.AsyncSessionFactory") as mock_factory:
            ...
            for i in range(PMS_CIRCUIT_BREAKER_FAILURE_THRESHOLD):
                await audit_logger.log_event(event_type=AuditEventType.RATE_LIMIT_EXCEEDED, user_id=f"user{i}")

        # Escribir múltiples eventos usando fallback
        for i in range(3):
            await audit_logger.log_event(
                event_type=AuditEventType.ESCALATION, user_id=f"fallback_{i}", details={"index": i}
            )
...
async def test_metrics_updated_correctly():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("app.services.security.audit_logger.AsyncSessionFactory") as mock_factory:
            ...
            await audit_logger.log_event(event_type=AuditEventType.LOGIN_SUCCESS, user_id="metrics_user")
...
async def test_no_exception_propagation_to_caller():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("app.services.security.audit_logger.AsyncSessionFactory") as mock_factory:
            ...
            try:
                await audit_logger.log_event(event_type=AuditEventType.PMS_ERROR, user_id="error_user")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("app.services.security.audit_logger.AsyncSessionFactory") as mock_factory:
            ...
            await asyncio.gather(
                *[
                    audit_logger.log_event(event_type=AuditEventType.DATA_ACCESS, user_id=f"concurrent_user_{i}")
                    for i in range(5)
                ]
            )
...
async def test_circuit_breaker_failure_count_resets_on_success():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("app.services.security.audit_logger.AsyncSessionFactory"):
            mock_session = AsyncMock()
            mock_session.add = MagicMock()
            ...
            mock_session.rollback = AsyncMock()
            for i in range(2):
                await audit_logger.log_event(event_type=AuditEventType.DATA_ACCESS, user_id=f"fail_{i}")

            # 1 éxito (resetea contador)
            mock_session.commit = AsyncMock(return_value=None)
            await audit_logger.log_event(event_type=AuditEventType.LOGIN_SUCCESS, user_id="success_user")

            # 2 fallos más (contador reseteado, no llega a threshold de 5)
            mock_session.commit = AsyncMock(side_effect=SQLAlchemyError("Error"))
            mock_session.rollback = AsyncMock()
            for i in range(2):
                await audit_logger.log_event(event_type=AuditEventType.DATA_ACCESS, user_id=f"fail2_{i}")

...
class TestReservationManagement:
    ...
    async def test_update_reservation(self, pms_service, sample_guest, sample_reservation_data):
        ...
        update_data = {
            "adults": 3,
            "special_requests": ["Ocean view", "Extra towels"]
        }
        
        updated_reservation = await pms_service.update_reservation(
            created_reservation.reservation_id, update_data
        )
    ...
    async def test_cancel_reservation(self, pms_service, sample_guest, sample_reservation_data):
        ...
        # Cancel reservation
        success = await pms_service.cancel_reservation(
            created_reservation.reservation_id, "Test cancellation"
        )
...
class TestReservationWorkflow:
    ...
    async def test_complete_reservation_workflow(self, reservation_manager):
        ...
        context_confirmed = ConversationContext(
            session_id=session_id,
            reservation_context={
                **context_with_guest.reservation_context,
                "confirmed": True
            }
        )
...
class TestConfirmationService:
    ...
    async def test_generate_confirmation_documents(self, confirmation_service, sample_guest, sample_reservation_data):
        ...
        guest_preferences = GuestPreferences(
            ...
            preferred_channels=[ConfirmationChannel.EMAIL]
        )
        
        documents = await confirmation_service._generate_confirmation_documents(
            reservation, guest_preferences
        )
    ...
    async def test_send_confirmation_email_channel(self, confirmation_service, sample_guest, sample_reservation_data):
        ...
        result = await confirmation_service.send_confirmation(
            ...
            GuestPreferences(preferred_language="en")
        )
...
class TestEndToEndIntegration:
    ...
    async def test_full_hotel_agent_workflow(self, pms_service, reservation_manager, confirmation_service):
        ...
        state, data = await reservation_manager.process_workflow_step(
            workflow.workflow_id, inquiry_context
        )
        ...
        state, data = await reservation_manager.process_workflow_step(
            workflow.workflow_id, selection_context
        )
        ...
        state, data = await reservation_manager.process_workflow_step(
            workflow.workflow_id, guest_context
        )
        ...
        confirmation_context = ConversationContext(
            session_id=session_id,
            reservation_context={
                **guest_context.reservation_context,
                "confirmed": True
            }
        )
        ...
        state, data = await reservation_manager.process_workflow_step(
            workflow.workflow_id, confirmation_context
        )
    ...
    async def test_error_handling_and_recovery(self, pms_service, reservation_manager):
        ...
        state, data = await reservation_manager.process_workflow_step(
            workflow.workflow_id, invalid_context
        )
        ...
        state, data = await reservation_manager.process_workflow_step(
            workflow.workflow_id, valid_context
        )
    ...
    async def test_performance_metrics(self, pms_service):
        ...
        for i in range(5):
            task = asyncio.create_task(
                pms_service.check_availability(
                    checkin_date=tomorrow,
                    checkout_date=day_after,
                    adults=2,
                    children=0
...
if __name__ == "__main__":
    ...
    pytest.main([
        ...
        "-v",
        "--tb=short",
        "-k", "test_complete_reservation_workflow or test_full_hotel_agent_workflow"
    ])
                ...
                "changes": [
                    {
                        "value": {
                            ...
                            "contacts": [{"profile": {"name": "John Doe"}, "wa_id": "14155552671"}],
                            "messages": [
                                {
                                    "from": "14155552671",
                ...
                "changes": [
                    {
                        "value": {
                            "messaging_product": "whatsapp",
                            "metadata": {"display_phone_number": "15551234567", "phone_number_id": "PHONE_NUMBER_ID"},
                            "contacts": [{"profile": {"name": "Jane Smith"}, "wa_id": "14155552672"}],
                            "messages": [
                                {
                                    "from": "14155552672",
                                    "id": "wamid.AUDIO123",
...
async def test_whatsapp_audio_message_e2e(sample_whatsapp_webhook_audio):
    ...
    with (
        patch("app.services.whatsapp_client.WhatsAppMetaClient") as MockClient,
        patch("app.routers.webhooks.settings") as mock_settings,
        patch("app.routers.webhooks.get_redis") as mock_redis,
    ):
...
async def test_whatsapp_webhook_verification_invalid_token():
    ...
    with patch("app.routers.webhooks.settings") as mock_settings:
        ...
        async with AsyncClient(app=app, base_url="http://test") as client:
            response = await client.get(
                "/webhooks/whatsapp",
                params={"hub.mode": "subscribe", "hub.verify_token": "wrong_token", "hub.challenge": "challenge_12345"},
            )
...
async def test_whatsapp_template_message_flow():
    ...
    with patch("app.services.whatsapp_client.WhatsAppMetaClient") as MockClient:
        client_instance = AsyncMock()
        client_instance.send_template_message.return_value = {"messages": [{"id": "wamid.template123"}]}
        MockClient.return_value = client_instance

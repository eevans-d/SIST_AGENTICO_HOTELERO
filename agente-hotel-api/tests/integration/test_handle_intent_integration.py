...
def orchestrator(mock_pms_adapter, mock_session_manager, mock_lock_service):
    ...
    return Orchestrator(
        pms_adapter=mock_pms_adapter, session_manager=mock_session_manager, lock_service=mock_lock_service
    )
    ...
    async def test_business_hours_check_during_hours(
        ...
        with patch("app.services.orchestrator.datetime") as mock_datetime:
            ...
            result = await orchestrator.handle_intent(
                nlp_result=nlp_result, session=sample_session, message=sample_message
            )
...
class TestAvailabilityFeature:
    ...
    async def test_availability_with_dates(
        ...
        mock_pms_adapter.check_availability.return_value = {
            "available": True,
            "rooms": [{"id": "101", "type": "double", "price": 80.00}, {"id": "102", "type": "suite", "price": 150.00}],
        }
...
class TestQRCodeFeature:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_qr_code_generation(self, orchestrator, mock_session_manager, sample_message, sample_session):
        """Test: Generación de código QR"""
        sample_session["reservation_id"] = "RES-12345"
        mock_session_manager.get_session.return_value = sample_session

        nlp_result = {"intent": "generar_qr", "confidence": 0.80, "entities": {}}
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_low_confidence_fallback(self, orchestrator, mock_session_manager, sample_message, sample_session):
...
class TestIntentRouting:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_unknown_intent_handling(self, orchestrator, mock_session_manager, sample_message, sample_session):
        """Test: Intent desconocido debe manejarse gracefully"""
        mock_session_manager.get_session.return_value = sample_session

        nlp_result = {"intent": "intent_that_does_not_exist", "confidence": 0.75, "entities": {}}

...
async def test_execute_optimization_endpoint(async_test_client):
    ...
    response = await async_test_client.post(
        "/api/v1/performance/optimization/execute",
        params={"force": True}
    )
...
async def test_error_handling_in_optimization():
    ...
    try:
        ...
        with pytest.raises(Exception):
            await optimizer.execute_optimization(
                force=True,
                optimization_types=['invalid_type']
            )
...
async def test_optimization_state_persistence():
    ...
    try:
        ...
        result = await optimizer.execute_optimization(force=True)

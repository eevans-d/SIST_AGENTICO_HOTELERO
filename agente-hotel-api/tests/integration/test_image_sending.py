...
async def orchestrator(mock_redis):
    ...
    pms_adapter = Mock()
    pms_adapter.check_availability = AsyncMock(return_value={
        "available": True,
        "rooms": [{"type": "double", "price": 10000}]
    })
...
class TestAvailabilityWithRoomPhoto:
    ...
    async def test_sends_image_after_availability_check(self, mock_settings, orchestrator):
        ...
        with patch.object(orchestrator.nlp_engine, "process_text", return_value={
    ...
    async def test_works_with_audio_messages(self, mock_settings, orchestrator):
        ...
        # Mock STT
        with patch.object(orchestrator.audio_processor, "transcribe_audio", return_value={
            "transcript": "Check availability for suite",
            "confidence": 0.90,
            "language": "en"
        }):
            # Mock NLP

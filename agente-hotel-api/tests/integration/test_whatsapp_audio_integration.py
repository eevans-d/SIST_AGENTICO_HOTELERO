...
async def whatsapp_client():
    ...
    client.send_audio_message.return_value = {
        "message_id": "whatsapp_message_123456",
        "status": "sent"
    }
...
async def test_whatsapp_audio_download_integration(whatsapp_client):
    ...
    whatsapp_client.download_media.assert_called_once_with(
        unified_message.metadata["audio"]["media_id"]
    )
...
async def test_whatsapp_audio_download_error_handling(whatsapp_client):
    ...
    whatsapp_client.download_media.side_effect = AudioDownloadError(
        "Error al descargar audio: status code 404"
    )
...
async def test_whatsapp_audio_send_integration(whatsapp_client):
    ...
    # Verificar que se llamó correctamente
    whatsapp_client.send_audio_message.assert_called_once_with(
        recipient, audio_data
    )
...
async def test_end_to_end_whatsapp_audio_flow(whatsapp_client, audio_processor):
    ...
    audio_path = await whatsapp_client.download_media(
        unified_message.metadata["audio"]["media_id"]
    )
...
async def test_whatsapp_error_handling_with_fallback(whatsapp_client, audio_processor):
    ...
    whatsapp_client.send_audio_message.side_effect = AudioProcessingError(
        "Error al enviar mensaje de audio"
    )
    whatsapp_client.send_message.return_value = {
        "message_id": "text_fallback_123",
        "status": "sent"
    }

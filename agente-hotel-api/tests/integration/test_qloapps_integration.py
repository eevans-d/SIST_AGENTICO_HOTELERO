...
class TestQloAppsClient:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(settings.pms_type != "qloapps", reason="Requires real QloApps connection (PMS_TYPE=qloapps)")
    ...
    async def test_connection_mock(self):
        ...
        with patch("httpx.AsyncClient.get") as mock_get:
            mock_get.return_value = AsyncMock(status_code=200, json=lambda: {"prestashop": {"api": "1.0"}})
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(settings.pms_type != "qloapps", reason="Requires real QloApps connection")
    ...
    async def test_check_availability_mock(self):
        ...
        mock_response = {"rooms": [{"id": 1, "name": "Standard Room", "available": 5, "price": 100.00}]}
        ...
        with patch("httpx.AsyncClient.get") as mock_get:
            ...
            try:
                ...
                rooms = await client.check_availability(
                    hotel_id=1, date_from=check_in, date_to=check_out, num_rooms=1, num_adults=2
                )
    ...
    async def test_authentication_error(self):
        ...
        with patch("httpx.AsyncClient.get") as mock_get:
            mock_get.return_value = AsyncMock(status_code=401, text="Unauthorized")
...
class TestQloAppsAdapter:
    ...
    async def test_create_reservation_flow(self, pms_adapter):
        ...
        guest_info = {"name": "Test Guest", "email": "test@example.com", "phone": "+1234567890"}
        ...
        with patch.object(pms_adapter.client, "search_customer_by_email") as mock_search:
            with patch.object(pms_adapter.client, "create_customer") as mock_create_customer:
                with patch.object(pms_adapter.client, "create_booking") as mock_create_booking:
                    ...
                    # Booking creation
                    mock_create_booking.return_value = {"id": 456, "reference": "BOOK456", "status": "confirmed"}
    ...
    async def test_circuit_breaker_opens_on_failures(self, pms_adapter):
        ...
        with patch.object(pms_adapter.client, "check_availability") as mock_check:
            ...
            for _ in range(6):  # Threshold is 5
                try:
                    await pms_adapter.check_availability(check_in="2025-01-10", check_out="2025-01-12", guests=2)
                except PMSError:
                    pass

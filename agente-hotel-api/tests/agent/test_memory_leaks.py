...
class MemorySnapshot:
    ...
    def __repr__(self):
        return (f"MemorySnapshot(rss={self.rss_mb:.2f}MB, heap={self.heap_mb:.2f}MB, "
...
def analyze_memory_growth(snapshots: List[MemorySnapshot]) -> Dict:
    ...
    # Detectar leaks
    has_leak = (
        rss_growth_mb > MEMORY_THRESHOLD_MB or
        rss_growth_rate > GROWTH_RATE_THRESHOLD
    )
...
async def test_concurrent_conversations_no_leak(nlp_engine, process):
    ...
    for round_num in range(LEAK_ITERATIONS // 10):
        ...
        tasks = [
            conversation(f"concurrent_user_{round_num}_{i}")
            for i in range(10)
        ]
...
async def test_no_performance_degradation_over_time(nlp_engine):
    ...
    assert degradation_pct < 20, \
        f"❌ PERFORMANCE DEGRADATION: {degradation_pct:.1f}% (threshold: 20%)"

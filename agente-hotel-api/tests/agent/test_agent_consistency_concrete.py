...
class TestResponseDeterminism:
    ...
    async def test_availability_query_intent_consistency(self, nlp_engine):
        ...
        for intent in intents:
            assert intent in ["check_availability", "inquire", "availability"], \
    ...
    async def test_pricing_response_template_consistent(self, orchestrator):
        ...
        assert len(unique_hashes) <= 2, \
            f"Too many response variations: {len(unique_hashes)}"
    ...
    async def test_error_messages_deterministic(self, orchestrator):
        ...
        assert len(set(error_responses)) == 1, \
            f"Inconsistent error messages: {set(error_responses)}"
    ...
    async def test_language_detection_consistent(self, nlp_engine):
        ...
        for expected_lang, text in test_cases.items():
            ...
            assert len(set(detected_langs)) == 1, \
                f"Inconsistent language detection: {set(detected_langs)}"
            assert detected_langs[0] == expected_lang, \
                f"Wrong language: {detected_langs[0]} vs {expected_lang}"
...
class TestContextConsistency:
    ...
    async def test_context_preserved_between_messages(self, orchestrator, session_manager):
        ...
        assert session.get("guest_count") == 2 or session.get("guests") == 2, \
            "Guest count not in session"
    ...
    async def test_multi_intent_context_handling(self, orchestrator):
        ...
        response = result2['response'].lower()
        assert any(word in response for word in ["precio", "cuesta", "€", "$"]), \
            "Didn't answer pricing question"
...
class TestTemporalConsistency:
    ...
    async def test_date_validation_stable(self, nlp_engine):
        ...
        assert all(v == False for v in validations) or \
               all("past" in str(v).lower() for v in validations), \
    ...
    async def test_special_characters_stable(self, nlp_engine):
        ...
        for text in texts_with_special:
            ...
            assert len(set(intents)) == 1, \
                f"Inconsistent intent with special chars: {set(intents)}"

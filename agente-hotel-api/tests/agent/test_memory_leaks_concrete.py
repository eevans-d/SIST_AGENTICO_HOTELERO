...
class TestNLPEngineMemoryLeaks:
    ...
    async def test_nlp_repeated_processing_no_leak(self, nlp_engine):
        ...
        assert memory_growth_mb < 10.0, f"Memory leak detected: grew {memory_growth_mb:.2f} MB (limit: 10 MB)"

        # Validación: Peak memory reasonable
        peak_mb = peak / 1024 / 1024
        assert peak_mb < 50.0, f"Peak memory too high: {peak_mb:.2f} MB"
    ...
    async def test_nlp_intent_cache_bounded(self, nlp_engine):
        ...
        if hasattr(nlp_engine, "_intent_cache"):
            ...
            assert cache_size < 1000, f"Intent cache unbounded: {cache_size} entries (limit: 1000)"
    ...
    async def test_nlp_entity_extraction_no_accumulation(self, nlp_engine):
        ...
        assert object_growth_pct < 20.0, f"Object accumulation detected: {object_growth_pct:.1f}% growth (limit: 20%)"
...
class TestSessionManagerMemoryLeaks:
    ...
    async def test_session_creation_destruction_no_leak(self, session_manager):
        ...
        assert memory_growth_mb < 5.0, f"Session Manager leak: grew {memory_growth_mb:.2f} MB (limit: 5 MB)"

        # Validación: Sessions active count = 0
        active_count = await session_manager.get_active_session_count()
        assert active_count == 0, f"Sessions not cleaned: {active_count} still active"
    ...
    async def test_session_timeout_cleanup(self, session_manager):
        ...
        if hasattr(session_manager, "_cleanup_expired_sessions"):
            ...
            gc.collect()
            active_count = await session_manager.get_active_session_count()
            assert active_count == 0, f"Session timeout cleanup failed: {active_count} still active"
    ...
    async def test_concurrent_session_access_no_leak(self, session_manager):
        ...
        tasks = [access_session(f"concurrent_user_{i}", 20) for i in range(50)]
        ...
        # Validación: Growth < 10 MB
        assert memory_growth_mb < 10.0, f"Concurrent access leak: grew {memory_growth_mb:.2f} MB (limit: 10 MB)"
...
class TestPMSAdapterMemoryLeaks:
    ...
    async def test_pms_repeated_api_calls_no_leak(self, pms_adapter):
        ...
        for i in range(500):
            ...
            try:
                result = await pms_adapter.check_availability(
                    check_in=check_in, check_out=check_out, room_type="deluxe"
                )
        ...
        # Validación: Growth < 10 MB
        assert memory_growth_mb < 10.0, f"PMS Adapter leak: grew {memory_growth_mb:.2f} MB (limit: 10 MB)"
    ...
    async def test_pms_cache_bounded(self, pms_adapter):
        ...
        for i in range(1000):
            ...
            try:
                await pms_adapter.check_availability(check_in=check_in, check_out=check_out, room_type="standard")
            except Exception:
                pass
        ...
        if hasattr(pms_adapter, "_response_cache"):
            ...
            assert cache_size < 500, f"PMS cache unbounded: {cache_size} entries (limit: 500)"
    ...
    async def test_pms_connection_pool_stable(self, pms_adapter):
        ...
        async def make_request(i: int):
            try:
                await pms_adapter.check_availability(check_in="2025-02-01", check_out="2025-02-05", room_type="suite")
            except Exception:
                pass
        ...
        if hasattr(pms_adapter, "_http_client"):
            ...
            if hasattr(client, "_pool"):
                ...
                if hasattr(pool, "_connections"):
                    ...
                    assert active_connections < 20, f"Connection pool leak: {active_connections} idle connections"
...
class TestAudioProcessorMemoryLeaks:
    ...
    async def test_audio_processing_repeated_no_leak(self):
        ...
        assert memory_growth_mb < 15.0, f"Audio Processor leak: grew {memory_growth_mb:.2f} MB (limit: 15 MB)"
...
class TestConcurrentConversationsStress:
    ...
    async def test_concurrent_users_no_leak(self, orchestrator):
        ...
        tasks = [simulate_user_conversation(f"stress_user_{i}") for i in range(100)]
        ...
        assert memory_growth_mb < 50.0, f"Concurrent users leak: grew {memory_growth_mb:.2f} MB (limit: 50 MB)"
        ...
        if latencies:
            ...
            assert p95_latency < 5.0, f"P95 latency degraded: {p95_latency:.2f}s (limit: 5s)"
...
class TestPerformanceDegradation:
    ...
    async def test_gc_overhead_acceptable(self):
        ...
        for i in range(1000):
            ...
            data = {"id": i, "payload": list(range(100)), "nested": {"a": i, "b": i * 2, "c": [i] * 10}}
        ...
        assert gc_overhead_pct < 5.0, f"Excessive GC overhead: {gc_overhead_pct:.2f}% (limit: 5%)"
    ...
    async def test_object_count_stable_after_gc(self):
        ...
        for count in last_three:
            ...
            assert delta_pct < 10.0, f"Object count unstable: {delta_pct:.1f}% variation (limit: 10%)"

        # Validar no crecimiento excesivo vs baseline
        final_growth_pct = ((object_counts[-1] - baseline_objects) / baseline_objects) * 100
        assert final_growth_pct < 50.0, f"Object count grew excessively: {final_growth_pct:.1f}% (limit: 50%)"

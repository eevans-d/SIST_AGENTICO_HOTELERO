...
async def test_agent_consistency(consistency_tester, scenario):
    ...
    assert result['std_similarity'] < 0.15, \
        f"Desviación estándar muy alta: {result['std_similarity']:.4f}"
...
async def test_consistency_with_context(consistency_tester):
    ...
    result = await consistency_tester.test_scenario(
        prompt="¿Puedo hacer late checkout?",
        iterations=50,
        context=context
    )
    
    print(f"\n{'='*60}")
    print(f"Test: Consistencia CON contexto")
    print(f"{'='*60}")
    ...
    assert result['avg_similarity'] >= 0.90, \
        "Con contexto fijo, la similitud debe ser >90%"
    
    assert result['unique_responses'] <= 5, \
        "Con contexto fijo, no debería haber más de 5 respuestas únicas"
...
async def test_consistency_temporal(agent):
    ...
    print(f"\n{'='*60}")
    print(f"Test: Consistencia TEMPORAL")
    print(f"{'='*60}")
    ...
    assert similarity_temporal >= 0.80, \
        f"Drift temporal detectado: similitud entre batches = {similarity_temporal:.2%}"
...
async def test_consistency_under_load(agent):
    ...
    tasks = [
        agent.chat(prompt, user_id=f"concurrent_{i}")
    ...
    successful_responses = [
        r.get('response', '') if isinstance(r, dict) else str(r)
        for r in results
    ...
    print(f"\n{'='*60}")
    print(f"Test: Consistencia BAJO CARGA")
    print(f"{'='*60}")
    ...
    if len(successful_responses) >= 2:
        ...
        similarities = [
            SequenceMatcher(None, baseline.lower(), resp.lower()).ratio()
        ...
        assert avg_sim >= 0.75, \
            f"Consistencia degradada bajo carga: {avg_sim:.2%} < 75%"
    
    # No más de 10% de errores
    error_rate = len(errors) / concurrent_requests
    assert error_rate < 0.10, \
        f"Tasa de error muy alta bajo carga: {error_rate:.2%}"

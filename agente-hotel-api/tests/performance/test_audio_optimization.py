...
from app.services.audio_compression_optimizer import (
    AudioCompressionOptimizer,
    CompressionLevel,
    NetworkConditions
)
from app.services.audio_connection_pool import (
...
class TestAudioCompressionOptimizer:
    ...
    async def test_compression_levels_performance(self, compression_optimizer, sample_audio_data):
        ...
        for level, result in results.items():
            if result["success"]:
                print(f"  {level}: {result['time']:.3f}s, ratio: {result['ratio']:.2f}, quality: {result['quality']:.2f}")
    ...
    async def test_adaptive_compression_network_conditions(self, compression_optimizer, sample_audio_data):
        ...
        assert cellular_result["ratio"] >= ethernet_result["ratio"], \
            "Conexión celular debería usar más compresión"
        ...
        with patch('aiohttp.ClientSession') as mock_session:
            ...
            with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
                ...
                try:
                    ...
                    await optimized_processor._download_audio_optimized(
...
async def test_optimization_integration():
    ...
    try:
        ...
        cached_result = await cache_optimizer.get(
            "integration_test",
            AudioCacheType.PROCESSED_AUDIO
        )

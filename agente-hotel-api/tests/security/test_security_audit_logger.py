...
class TestSecurityEventLogging:
    ...
    async def test_log_event_with_metadata(self, audit_logger):
        ...
        event_with_metadata = {
            ...
            "metadata": {"attempted_resource": "/admin/users", "user_role": "guest", "required_role": "admin"},
        }
...
class TestSecurityMetrics:
    ...
    async def test_collect_security_metrics(self, audit_logger):
        ...
        audit_logger.redis_client.get = AsyncMock(
    ...
    async def test_metrics_time_series(self, audit_logger):
        ...
        time_series_data = [
            json.dumps({"timestamp": "2024-01-01T10:00:00Z", "event_count": 10, "threat_level": "medium"}),
            json.dumps({"timestamp": "2024-01-01T11:00:00Z", "event_count": 15, "threat_level": "high"}),
        ]
...
class TestDataRetention:
    ...
    async def test_cleanup_old_events(self, audit_logger):
        ...
        old_events = [
            json.dumps(
                {"event_id": f"event-{i}", "timestamp": (datetime.now(timezone.utc) - timedelta(days=40)).isoformat()}
            )
            for i in range(10)
        ]
...
class TestSecurityReporting:
    ...
    async def test_compliance_report(self, audit_logger):
        ...
        audit_logger.redis_client.lrange = AsyncMock(
            return_value=[
                json.dumps(
                    {
        ...
        compliance_report = await audit_logger.generate_compliance_report(
            start_date=datetime.now(timezone.utc) - timedelta(days=30), end_date=datetime.now(timezone.utc)
        )
    ...
    async def test_threat_intelligence_report(self, audit_logger):
        ...
        audit_logger.redis_client.hgetall = AsyncMock(return_value={"192.168.1.100": json.dumps(threat_data[0])})

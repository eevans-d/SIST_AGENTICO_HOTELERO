...
class TestSecurityIntegration:
    ...
    def integrated_app(self):
        ...
        async def get_profile(current_user=Depends(get_current_user)):
            return {"user_id": current_user["id"], "username": current_user["username"], "roles": current_user["roles"]}
        ...
        async def create_reservation(reservation_data: dict, current_user=Depends(get_current_user)):
            # Encrypt sensitive data
            app.state.encryption_service.encrypt(json.dumps(reservation_data))

            return {"reservation_id": "RES-123", "status": "created", "user_id": current_user["id"]}

        @app.get("/api/reservations/{reservation_id}")
        async def get_reservation(reservation_id: str, current_user=Depends(get_current_user)):
    ...
    def test_privilege_escalation_prevention(self, integrated_app):
        ...
        admin_endpoints = ["/api/admin/users", "/api/admin/security/audit", "/api/admin/roles"]
...
class TestConcurrentSecurityOperations:
    ...
    def concurrent_app(self):
        ...
        async def concurrent_login(credentials: dict):
            ...
            with app.state.session_lock:
                app.state.active_sessions[session_id] = {"username": username, "created_at": time.time()}
    ...
    def test_concurrent_logins(self, concurrent_app):
        ...
        def login_user(username):
            return concurrent_app.post("/api/auth/login", json={"username": username, "password": "password"})
    ...
    def test_rate_limiting_under_load(self, concurrent_app):
        ...
        def attempt_login():
            return concurrent_app.post(
                "/api/auth/login", json={"username": "bruteforce_target", "password": "wrong_password"}
            )
    ...
    def test_session_cleanup_race_conditions(self, concurrent_app):
        ...
        sessions = []
        for i in range(5):
            response = concurrent_app.post("/api/auth/login", json={"username": f"user_{i}", "password": "password"})
    ...
    def recovery_app(self):
        ...
        async def login_with_failures(credentials: dict):
            ...
            if app.state.redis_failure:
                ...
                return {"access_token": "fallback_token", "warning": "Using fallback authentication"}

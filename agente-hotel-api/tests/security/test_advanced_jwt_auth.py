...
from app.security.advanced_jwt_auth import (
    AdvancedJWTAuth, UserRole, UserRegistration, UserLogin,
    User, Session, AuthenticationError, AuthorizationError
)
...
class TestUserAuthentication:
    ...
    async def test_authenticate_user_account_locked(self, jwt_auth, sample_user_login, sample_user):
        ...
        jwt_auth._get_user_by_email = AsyncMock(return_value=sample_user)
        jwt_auth._check_account_lockout = AsyncMock(return_value={
            "locked": True,
...
class TestTokenGeneration:
    ...
    async def test_verify_valid_token(self, jwt_auth, sample_user):
        ...
        jwt_auth._get_session = AsyncMock(return_value={
    ...
    async def test_refresh_access_token(self, jwt_auth, sample_user):
        ...
        jwt_auth._validate_refresh_token = AsyncMock(return_value={
            "valid": True,
            "user_id": sample_user.user_id,
            "session_id": "session-123"
        })
...
class TestSessionManagement:
    ...
    async def test_create_session(self, jwt_auth, sample_user):
        ...
        session_id = await jwt_auth._create_session(
            sample_user.user_id,
            "127.0.0.1",
...
class TestErrorHandling:
    ...
    async def test_redis_connection_error(self, jwt_auth, sample_user):
        ...
        result = await jwt_auth._create_session(
            sample_user.user_id,
            "127.0.0.1",
            "Test Browser"
        )
...
class TestPerformanceAndScaling:
    ...
    async def test_concurrent_login_attempts(self, jwt_auth, sample_user_login, sample_user):
        ...
        import asyncio
        tasks = [
            jwt_auth.authenticate_user(sample_user_login, f"127.0.0.{i}")
            for i in range(10)
        ]

...
|-----------|--------|---------|-------|
| **API Server** | 🟢 Running | 0.1.0 | Docker Container |
| **Health Checks** | 🟢 All Pass | - | /live=200, /ready=200 |
| **Postgres** | 🟢 Connected | 14-alpine | Supabase / Managed Postgres |
| **Redis** | 🟢 Connected | 7-alpine | Managed Redis |
...
- [x] App container created and configured
- [x] Postgres database available
...
1. Configure real WhatsApp credentials
   ```bash
   (Comando para configurar secretos) \
     WHATSAPP_ACCESS_TOKEN=... \
     WHATSAPP_PHONE_NUMBER_ID=... \
     WHATSAPP_VERIFY_TOKEN=... \
     WHATSAPP_APP_SECRET=... \
...
3. Configure real PMS endpoint
   ```bash
   (Comando para configurar secretos) PMS_TYPE=qloapps -a agente-hotel-api
   (Comando para configurar secretos) PMS_BASE_URL=https://qloapps.yourdomain.com -a agente-hotel-api
   ```
...
2. **Load Testing**
   ...
   ab -n 1000 -c 10 https://(APP_URL)/health/live
   ```
...
1. **Deploy Optimized Docker Image**
   ```bash
   # Update configuration to use Dockerfile.optimized
   (Comando de deploy)
   ```
...
1. **Final Validation**
   ```bash
   curl -v https://(APP_URL)/health/ready
   ```
